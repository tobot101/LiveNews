...
function buildLocalQueryVariants(city, state, stateNameLookup) {
  ...
  const withShortRecency = baseVariants.slice(0, 4).map((value) => `${value} when:2d`);
  const withPublicWindowRecency = baseVariants.slice(0, 4).map((value) => `${value} when:7d`);
  return Array.from(new Set([...baseVariants, ...withShortRecency, ...withPublicWindowRecency]));
}
