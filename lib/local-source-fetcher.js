...
const {
  ...
  classifyInputSignal,
} = require("./local-signal-classifier");
const { clusterInputSignal } = require("./local-story-clustering");
...
function classifyFetchedSignal(signalInput = {}, source = {}, paths = {}, options = {}) {
  if (options.classifySignals === false) return signalInput;
  try {
    const classification = classifyInputSignal(signalInput, {
      ...options,
      paths,
      sources: source?.id ? [source] : undefined,
    });
    return applySignalClassification(signalInput, classification);
  } catch {
    return signalInput;
  }
}

function clusterFetchedSignal(signalInput = {}, source = {}, paths = {}, options = {}) {
  if (options.clusterSignals === false) return null;
  try {
    return clusterInputSignal(signalInput, {
      ...options,
      paths,
      sources: source?.id ? [source] : undefined,
    });
...
async function fetchSourceFeed(entry = {}, options = {}) {
  ...
  try {
    ...
    for (const item of fetched.items || []) {
      const classifiedItem = classifyFetchedSignal(item, source, paths, options);
      const created = createInputSignal(classifiedItem, { paths, now: options.now });
      createdSignals.push(created);
      if (created.created) {
        const clustered = clusterFetchedSignal(created.signal, source, paths, options);
        if (clustered) clusteredSignals.push(clustered);
      }
    }
    ...
    return { status, source, feed, run, items: fetched.items || [], createdSignals, clusteredSignals };
  } catch (error) {
    const run = registry.logSourceFetchRun({
      source_feed_id: feed.id,
      started_at: startedAt,
