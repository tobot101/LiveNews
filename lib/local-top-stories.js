...
function getAttentionBonus(story = {}) {
  ...
  const shares = Number(story.shares || story.engagement?.shares || 0);
  const comments = Number(story.comments_count || story.commentsCount || story.engagement?.comments || 0);
  const hides = Number(story.hides || story.engagement?.hides || 0);
  const reports = Number(story.reports || story.engagement?.reports || 0);
  const score =
    views * 0.03 +
    clicks * 0.65 +
    saves * 0.7 +
    ...
    hides * 1.2 -
    reports * 2.5;
  return Math.min(120, Math.max(0, Math.round(score)));
}
...
function scoreLocalTopStory(story = {}, now = new Date()) {
  ...
  return (
    (URGENCY_WEIGHTS[story.urgency] || 0) * 0.45 +
    getAttentionBonus(story) +
