...
function buildTitleBackedContext(core, socialHeadline, audiencePlan = {}) {
  ...
  if (category === "sports") {
    return `${sentence(socialHeadline)} The player, team, or season stakes are the main reason to follow the story.`;
  }
  if (audiencePlan.primaryHumanAngle && audiencePlan.primaryHumanAngle !== "reader clarity") {
    return `${sentence(socialHeadline)} The reader angle is ${audiencePlan.primaryHumanAngle}, with attribution to ${source}.`;
  }
  return `${sentence(socialHeadline)} The latest details are gathered with attribution to ${source}.`;
}
