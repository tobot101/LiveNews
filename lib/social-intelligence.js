...
const { explainCopyRisk } = require("./article-agents/copy-distance");
const { buildSourceFactMap } = require("./article-agents/source-fact-map");
const { runWriterRoom } = require("./article-agents/writer-room");
const { rewriteUntilPass } = require("./article-agents/writing-rewriter");
const {
...
function approvedSocialWritingIsWeak(core = {}) {
  ...
  const approvedText = approvedFields.join(" ");
  const status = cleanText(core.writingQualityStatus).toLowerCase();
  const title = cleanText(core.title).toLowerCase();
  const originalTitle = cleanText(core.originalTitle).toLowerCase();
  if (status && status !== "ready") return true;
  if (!approvedFields.length) return true;
  if (detectFallbackRisk(approvedText).risky) return true;
  if (title && originalTitle && title === originalTitle && !cleanText(core.approvedDescription)) return true;
  return false;
}

function stripSocialWriterText(value, exactArticleUrl = "") {
  return truncateWords(stripSocialBoilerplateForWriting(value, exactArticleUrl), 44);
}

function runOriginalSocialWriter(core = {}) {
  const story = buildSocialWritingStory(core);
  const factMap = buildSourceFactMap(story);
  const exactArticleUrl = cleanText(core.linkState?.exactArticleUrl || core.exactArticleUrl || "");
  const needsWriterRoom = approvedSocialWritingIsWeak(core);
  const socialRoom = needsWriterRoom ? runWriterRoom(story, "socialCaption") : null;
  const titleRoom = needsWriterRoom ? runWriterRoom(story, "title") : null;
  const writerText = stripSocialWriterText(socialRoom?.selectedCandidate?.text, exactArticleUrl);
  const titleText = cleanText(titleRoom?.selectedCandidate?.text || "").replace(/[.!?]$/, "");
  return {
    story,
    factMap,
    needsWriterRoom,
    ...
    writerText,
    titleText,
    status: needsWriterRoom
      ? socialRoom?.status === "passed" || titleRoom?.status === "passed"
        ? "used"
        : socialRoom?.status === "needs_more_context" || titleRoom?.status === "needs_more_context"
          ? "needs_more_context"
          : "blocked"
      : "not_needed",
    notes: [
      ...(socialRoom?.agentNotes || []),
      ...(titleRoom?.agentNotes || []),
    ],
  };
}
...
function buildSocialWritingQuality({ text, context, fieldName, exactArticleUrl }) {
  ...
  const copyRiskTeacher = writingTeachers.find((teacher) => teacher.name === "CopyRiskTeacher");
  const copyRisk = copyRiskTeacher?.copyDistance || explainCopyRisk(
    evaluationText || text,
    context.doNotSay || [],
    context
  );
...
function needsOriginalWriterRepair(writingQuality = {}) {
  const teachers = writingQuality.writingTeacherChecks || [];
  return teachers.some((teacher) =>
    ["StoryFocusTeacher", "CopyRiskTeacher"].includes(teacher.name) &&
      (teacher.passed === false || teacher.blocking === true)
  );
}

function summarizeWriterRoomNotes(originalWriter = {}) {
  return (originalWriter.notes || [])
    .map((note) => ({
      agent: cleanText(note.agent),
      status: cleanText(note.status),
      notes: (note.notes || []).map(cleanText).filter(Boolean).slice(0, 3),
    }))
    .filter((note) => note.agent || note.notes.length)
    .slice(0, 8);
}

function buildOriginalWriterVariantMetadata({
  writingSource = "approved_story",
  originalWriterStatus = "not_needed",
  rewriteSession = {},
  copyRisk = {},
  smartnessScore = {},
  writerRoomNotes = [],
...
function pickSocialReadyRewriteText(rewriteSession = {}, writingContext = {}, fieldName = "facebookCaption", exactArticleUrl = "") {
  const candidateTexts = uniqueBy(
    [
      rewriteSession.finalCandidate,
      ...(rewriteSession.attempts || []).flatMap((attempt) =>
        (attempt.candidates || []).map((candidate) => candidate.text)
      ),
    ].map((text) => stripSocialWriterText(text, exactArticleUrl)).filter(Boolean),
    (text) => text.toLowerCase()
  );
  const evaluated = candidateTexts
    .map((text) => ({
      text,
      quality: buildSocialWritingQuality({
        text,
    ...
    .filter((entry) => entry.quality.writingQualityStatus === "ready")
    .sort((left, right) => (right.quality.writingExam?.total || 0) - (left.quality.writingExam?.total || 0));
  return evaluated[0] || null;
}

function repairSocialVariantText({
  text,
  core,
  writingContext,
  fieldName,
  ...
  const initialQuality = buildSocialWritingQuality({
    text,
    context: writingContext,
    fieldName,
    exactArticleUrl,
  });
  const writerRoomNotes = summarizeWriterRoomNotes(originalWriter);
  const baseMetadata = buildOriginalWriterVariantMetadata({
    writingSource: originalWriter?.needsWriterRoom ? "writer_room" : "approved_story",
    originalWriterStatus: originalWriter?.status || "not_needed",
    copyRisk: initialQuality.copyRisk,
    smartnessScore: originalWriter?.socialRoom?.smartnessScore || {},
    writerRoomNotes,
  });
  const repairTriggered = needsOriginalWriterRepair(initialQuality);
  let rewriteSession = {};

  if (repairTriggered) {
    const rewriteInput = stripSocialBoilerplateForWriting(text, exactArticleUrl);
    rewriteSession = rewriteUntilPass(rewriteInput || text, originalWriter.factMap, "description", {
      maxRounds: 3,
      maxCandidatesPerRound: 5,
    });
    if (rewriteSession.status === "passed" || rewriteSession.attempts?.length) {
      const readyRewrite = pickSocialReadyRewriteText(rewriteSession, writingContext, fieldName, exactArticleUrl);
      if (readyRewrite) {
        const selectedDifferentAttempt = stripSocialWriterText(rewriteSession.finalCandidate, exactArticleUrl) !== readyRewrite.text;
        return {
          text: readyRewrite.text,
          writingQuality: readyRewrite.quality,
          metadata: buildOriginalWriterVariantMetadata({
            ...baseMetadata,
            writingSource: "rewrite_loop",
            originalWriterStatus: "used",
            rewriteSession,
            copyRisk: readyRewrite.quality.copyRisk || rewriteSession.copyRiskAfter,
            smartnessScore: originalWriter?.socialRoom?.smartnessScore || {},
            rewriteImprovementSummary: selectedDifferentAttempt
              ? `${rewriteSession.improvementSummary} Social caption selected a stronger ready candidate from the same rewrite session.`
              : rewriteSession.improvementSummary,
          }),
        };
      }
    }
  ...
  if (originalWriter?.needsWriterRoom && originalWriter.writerText) {
    const writerQuality = buildSocialWritingQuality({
      text: originalWriter.writerText,
      context: writingContext,
      fieldName,
      exactArticleUrl,
    });
    if (writerQuality.writingQualityStatus === "ready") {
      return {
        text: originalWriter.writerText,
        writingQuality: writerQuality,
        metadata: buildOriginalWriterVariantMetadata({
          ...baseMetadata,
          writingSource: "writer_room",
          originalWriterStatus: "used",
          rewriteSession,
          copyRisk: writerQuality.copyRisk,
          smartnessScore: originalWriter.socialRoom?.smartnessScore || {},
          rewriteImprovementSummary: "Used WriterRoom social context because approved writing was weak or too close to source wording.",
        }),
      };
    }
  }
  ...
  return {
    ...
    metadata: buildOriginalWriterVariantMetadata({
      ...baseMetadata,
      originalWriterStatus: rewriteSession.status === "needs_more_context"
        ? "needs_more_context"
        : originalWriter?.status === "needs_more_context"
          ? "needs_more_context"
          : "blocked",
      rewriteSession,
      rewriteImprovementSummary: rewriteSession.improvementSummary || "Original Writer could not safely repair this social variant.",
    }),
  };
}
...
function buildSocialCaptionVariants(core, audiencePlan = {}, memory = DEFAULT_SOCIAL_STYLE_MEMORY) {
  ...
  const originalWriter = runOriginalSocialWriter({
    ...core,
    publicSafetyRelevant,
  });
  const safetyFlags = {
  ...
  const socialHeadline = truncateWords(originalWriter.titleText || buildSocialHeadline(core), 22);
  ...
  let context = detectFallbackRisk(summary).risky
    ? fallbackContext
    : summary || fallbackContext;
  if (originalWriter.needsWriterRoom && originalWriter.writerText) {
    context = sentence(truncateWords(originalWriter.writerText, 38));
  }
  ...
  const facebookVariant = ({ id, label: variantLabel, captionShape, lead, body }) => {
    let variantLead = lead;
    let variantBody = body;
    let lines = [variantLead, variantBody, fbLinkLine, facebookHashtags].filter(Boolean);
    let message = lines.join("\n\n");
    let platformTeacherChecks = buildVariantTeacherChecks({
      text: message,
      exactArticleUrl,
    ...
    const repair = repairSocialVariantText({
    ...
    let writingQuality = repair.writingQuality;
    if (repair.text && repair.text !== [socialHeadline, lead, body].filter(Boolean).join(" ")) {
      const rewrittenSentence = sentence(truncateWords(repair.text, 44));
      if (captionShape === "source_first") {
        variantLead = `${sourceLine} ${rewrittenSentence}`;
        variantBody = "";
      } else if (captionShape === "concise_news") {
        variantLead = `Latest context: ${rewrittenSentence}`;
        variantBody = sourceLine;
      } else {
        variantLead = rewrittenSentence;
        variantBody = sourceLine;
      }
      lines = [variantLead, variantBody, fbLinkLine, facebookHashtags].filter(Boolean);
      message = lines.join("\n\n");
      platformTeacherChecks = buildVariantTeacherChecks({
        text: message,
        exactArticleUrl,
        source: sourceAttribution,
        safetyFlags,
      ...
      writingQuality = buildSocialWritingQuality({
        text: repair.text,
        context: writingContext,
        fieldName: "facebookCaption",
        exactArticleUrl,
    ...
    return {
      ...
      ...buildOriginalWriterVariantMetadata({
        ...repair.metadata,
        copyRisk: writingQuality.copyRisk || repair.metadata.copyRisk,
      }),
  ...
  const instagramVariant = ({ id, label: variantLabel, captionShape, shortTitle, captionLines, cardTitle, cardSubtitle }) => {
    let variantShortTitle = shortTitle;
    let variantCardTitle = cardTitle;
    let variantCardSubtitle = cardSubtitle;
    let caption = captionLines.filter(Boolean).join("\n");
    const repair = repairSocialVariantText({
    ...
    let writingQuality = repair.writingQuality;
    if (repair.text && repair.text !== [cardTitle, caption].filter(Boolean).join("\n")) {
      variantShortTitle = truncateWords(repair.text, 9);
      variantCardTitle = truncateWords(repair.text, 9);
      variantCardSubtitle = truncateWords(sourceLine, 12);
      const rewrittenSentence = sentence(truncateWords(repair.text, 44));
      const repairedCaptionLines = captionShape === "reader_impact"
        ? [rewrittenSentence, "", `LIVE NEWS • ${audiencePlan.postShape || label}`, `Source: ${source}.`, igLinkLine, hashtags]
        : captionShape === "short_news"
          ? [`Latest context: ${rewrittenSentence}`, "", `LIVE NEWS • ${label}`, `Source: ${source}.`, igLinkLine, hashtags]
          : ["LIVE NEWS", label, "", rewrittenSentence, "", `Source: ${source}.`, igLinkLine, hashtags];
      caption = repairedCaptionLines.filter(Boolean).join("\n");
      writingQuality = buildSocialWritingQuality({
        text: repair.text,
        context: writingContext,
        fieldName: "instagramCaption",
    ...
    const imagePlan = buildInstagramCardPlan({
      ...baseCardPlan,
      cardTitle: variantCardTitle,
      shortTitle: variantShortTitle,
      cardSubtitle: variantCardSubtitle,
      sourceLabel: sourceAttribution,
      exactArticleUrl,
      imageUrl: core.imageUrl,
      thumbnailUrl: core.thumbnailUrl,
      generatedCardUrl: core.generatedCardUrl,
      altText: `${variantShortTitle}. Live News coverage with attribution to ${sourceAttribution}.`,
    });
    ...
    return {
      ...
      cardSubtitle: variantCardSubtitle,
      altText: `${variantShortTitle}. Live News coverage with attribution to ${sourceAttribution}.`,
      storyText: `${variantShortTitle} ${variantCardSubtitle}`.trim(),
      renderStatus: imagePlan.renderStatus,
      carouselSlides: [
        { title: variantShortTitle, text: variantCardSubtitle },
