...
function isPublicSafetyRelevant(context = {}) {
  ...
  if (PUBLIC_SAFETY_EXPLICIT_TERMS.some((term) => text.includes(term))) return true;
...
function hasActivePublicSafetyAlert(context = {}) {
  const text = contextBodyText(context);
  return PUBLIC_SAFETY_EXPLICIT_TERMS.some((term) => text.includes(term));
}
...
function buildVariantTeacherChecks({ text, exactArticleUrl, source, safetyFlags, imagePlan, platform }) {
  ...
  if (platform === "instagram") {
    checks.push({
      ...
      passed: Boolean(imagePlan?.imageUrl || imagePlan?.generatedCardUrl),
    });
  }
  return checks;
}
...
function buildSocialCaptionVariants(core, audiencePlan = {}, memory = DEFAULT_SOCIAL_STYLE_MEMORY) {
  ...
  const imagePlan = {
    status: cleanText(core.imageUrl || core.generatedCardUrl) ? "durable_or_generated_image_available" : "generated_card_needed_before_instagram_publish",
    imageUrl: cleanText(core.imageUrl || ""),
    generatedCardUrl: cleanText(core.generatedCardUrl || ""),
    recommendedFormat: "1080x1080 square",
    requiredBeforeApiPublish: true,
  };
  ...
  const facebookVariant = ({ id, label: variantLabel, captionShape, lead, body }) => {
    ...
    const teacherChecks = buildVariantTeacherChecks({
      text: message,
      exactArticleUrl,
  ...
  const instagramVariant = ({ id, label: variantLabel, captionShape, shortTitle, captionLines, cardTitle, cardSubtitle }) => {
    ...
    return {
      ...
      cardTitle,
      cardSubtitle,
      altText: `${shortTitle}. Source-linked Live News coverage from ${sourceAttribution}.`,
      storyText: `${shortTitle} ${cardSubtitle}`.trim(),
      carouselSlides: [
        { title: shortTitle, text: cardSubtitle },
        { title: "Source-linked", text: `Coverage led by ${sourceAttribution}.` },
      ...
      safetyFlags,
      teacherChecks,
      publishable: basePublishable && Boolean(imagePlan.imageUrl || imagePlan.generatedCardUrl) && checksPass(teacherChecks),
      shape: captionShape,
      text: caption,
    };
  };
  ...
  const facebook = uniqueVariantList([
    ...
    facebookVariant({
      ...
      lead: title,
      body: titleAddsContext ? context : `${categoryLine}.`,
    }),
  ]);
