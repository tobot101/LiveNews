...
function containsForbiddenPrivateData(value, path = "") {
  ...
  return (
    ...
    /\b(a commenter said|comments said|fans are saying|internet reacts)\b/i.test(text) ||
    /\/admin\//i.test(text) ||
    /[?&](access_?token|api_?key|client_secret|secret|token)=/i.test(text)
  );
}
...
function combineMetricTotals(metrics = []) {
...
function getAttentionScore(metrics = {}, timeframe = "daily") {
  const clicks = safeNumber(metrics.clicks || metrics.linkClicks, 0);
  const views = safeNumber(metrics.views, 0);
  const velocity = safeNumber(metrics.engagementVelocity, 0);
  const socialScore = safeNumber(metrics.socialScore || metrics.score, 0);
  const sustainedDays = safeNumber(metrics.sustainedDays, 0);
  const clickWeight = timeframe === "weekly" ? 2.05 : 2.35;
  const velocityWeight = timeframe === "weekly" ? 7 : 10;
  const sustainedBonus = timeframe === "weekly" ? clamp(sustainedDays * 8, 0, 28) : clamp(sustainedDays * 4, 0, 12);
  return clamp(
    clicks * clickWeight +
      views * 0.08 +
...
function getAttentionBreakdown(metrics = {}, timeframe = "daily") {
  return {
    timeframe,
    views: Math.round(safeNumber(metrics.views, 0)),
    clicks: Math.round(safeNumber(metrics.clicks || metrics.linkClicks, 0)),
    engagementVelocity: Number(safeNumber(metrics.engagementVelocity, 0).toFixed(2)),
    socialScore: Number(safeNumber(metrics.socialScore || metrics.score, 0).toFixed(2)),
    sustainedDays: Number(safeNumber(metrics.sustainedDays, 0).toFixed(1)),
  };
}

function metricMatchesTopic(metric = {}, topic = {}, stories = []) {
  if (containsForbiddenPrivateData(metric)) return false;
  const metricStoryId = cleanText(metric.storyId || metric.articleId || "");
  const topicStoryIds = new Set((topic.relatedStoryIds || []).map(cleanText).filter(Boolean));
  if (metricStoryId && topicStoryIds.has(metricStoryId)) return true;

  const metricUrl = cleanText(metric.exactArticleUrl || metric.url || "").toLowerCase();
  const storyIdentities = new Set(
    (stories || [])
      .map(getStoryIdentityKey)
      .filter(Boolean)
  );
  if (metricUrl && [...storyIdentities].some((identity) => identity && metricUrl.includes(identity))) return true;

  const metricTopic = cleanText(metric.topic || metric.query || "");
  const metricKeywords = Array.isArray(metric.keywords) ? metric.keywords.map(cleanText) : extractKeywords(metricTopic, 8);
  const topicKeywords = [
    ...(topic.keywords || []),
    ...extractKeywords(topic.topicName || "", 8),
  ].map(cleanText);
  if (keywordOverlap(topicKeywords, metricKeywords) >= 0.28) return true;
  return false;
}

function topicSiteMetrics(topic = {}, stories = [], siteMetrics = []) {
  return (siteMetrics || []).filter((metric) => metricMatchesTopic(metric, topic, stories));
}

function topicAttentionMetrics(topic = {}, stories = [], siteMetrics = []) {
  return combineMetricTotals(topicSiteMetrics(topic, stories, siteMetrics));
}
...
function selectionBase({ label, story, topic, score, confidence, whySelected, duplicateStatus, freshnessWindow, sustainedDays, origin, attention }) {
  return {
    label,
    ...
    attentionScore: attention ? Math.round(clamp(attention.score, 0, 100)) : 0,
    attentionBreakdown: attention ? getAttentionBreakdown(attention.metrics, attention.timeframe) : getAttentionBreakdown({}, "daily"),
    story,
  };
}
...
function rankTopStoryOfDay(candidates = [], trendSignals = [], siteMetrics = [], options = {}) {
  ...
  const ranked = pool
    .map((story) => {
      ...
      const attentionScore = getAttentionScore(metrics, "daily");
      const tractionScore = clamp(attentionScore * 0.62 + dailyTrend * 0.38, 0, 100);
      const score =
        tractionScore * 0.5 +
        freshnessScore(story, 24, now) * 0.18 +
        sourceSignificanceScore(story) * 0.12 +
        storyQualityScore(story) * 0.08 +
        dailyTrend * 0.08 +
        clamp(safeNumber(options.editorBoosts?.[getStoryId(story)], story.editorBoost || 0), 0, 100) * 0.05;
      return { story, topic, score, attention: { score: tractionScore, metrics, timeframe: "daily" } };
    })
  ...
  const whySelected = `Chosen for today's homepage because it has the strongest current attention signal in the day window, balanced with freshness, source strength, and story quality.`;
...
function topicSustainedDays(topic, stories, siteMetrics = []) {
  ...
  const metricDays = Math.max(0, ...topicSiteMetrics(topic, stories, siteMetrics).map((metric) => safeNumber(metric.sustainedDays, 0)));
  const dates = (stories || [])
...
function rankStoryOfWeek(candidates = [], trendSignals = [], siteMetrics = [], options = {}) {
  ...
  const eligible = topics
    .map((topic) => {
      ...
      const topicMetrics = topicAttentionMetrics(topic, stories, siteMetrics);
      const topicAttentionScore = getAttentionScore(topicMetrics, "weekly");
      const weeklyTrend = trendStrength(topic.trendSignals, "weekly");
      const sortedStories = [...stories].sort(
        (a, b) => {
          const aMetrics = getMetricForStory(a, siteMetrics);
          const bMetrics = getMetricForStory(b, siteMetrics);
          const aAttention = getAttentionScore(aMetrics, "weekly");
          const bAttention = getAttentionScore(bMetrics, "weekly");
          const aComposite = aAttention * 0.5 + storyQualityScore(a) * 0.2 + sourceSignificanceScore(a) * 0.2 + freshnessScore(a, 24 * 7, now) * 0.1;
          const bComposite = bAttention * 0.5 + storyQualityScore(b) * 0.2 + sourceSignificanceScore(b) * 0.2 + freshnessScore(b, 24 * 7, now) * 0.1;
          return bComposite - aComposite || new Date(b.publishedAt) - new Date(a.publishedAt);
        }
      );
      ...
      const storyAttentionScore = getAttentionScore(metrics, "weekly");
      const weeklyTractionScore = clamp(Math.max(topicAttentionScore, storyAttentionScore) * 0.62 + weeklyTrend * 0.38, 0, 100);
      ...
      const score =
        weeklyTractionScore * 0.44 +
        clamp(sustainedDays * 20, 0, 100) * 0.18 +
        sourceSignificanceScore(bestStory) * 0.12 +
        storyQualityScore(bestStory) * 0.08 +
        freshnessScore(bestStory, 24 * 7, now) * 0.06 +
        weeklyTrend * 0.07 +
        clamp(safeNumber(bestStory.editorBoost || options.editorBoosts?.[getStoryId(bestStory)], 0), 0, 100) * 0.05 -
        duplicatePenalty -
      ...
      return {
        ...
        attention: {
          score: weeklyTractionScore,
          metrics: combineMetricTotals([topicMetrics, metrics]),
          timeframe: "weekly",
        },
      };
    })
  ...
  const whySelected =
    duplicateStatus === "same_topic_as_day"
      ...
      : "Chosen for the week because the topic has the strongest sustained attention across the week, with multi-day reader interest, source strength, and enough value to remain useful beyond a one-day spike.";
  return selectionBase({
