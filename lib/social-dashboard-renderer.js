...
function readinessNotes(variant, platform) {
  ...
  for (const check of checks) {
    if (check && check.passed === false) warnings.push(`${cleanText(check.message || check.id || "Readiness check")} needs attention`);
  }
  if (variant.publishable !== true) warnings.push("This draft is not ready to post yet.");
