...
function getDraftCaption(draft, platform) {
  ...
  return [title, summary, `Source: ${source}.`].filter(Boolean).join("\n\n");
}
...
function assertDraftCanPublish(draft, platform, options = {}, env = process.env) {
  ...
  if (draft?.supervisor?.shareableNow !== true) {
    failures.push("Draft supervisor must confirm the exact public Live News story page is shareable before API posting.");
  }
...
function buildInstagramPublishPlan(draft, options = {}, env = process.env) {
  ...
  const imagePlan = getInstagramImagePlan(draft, options.imageUrl);
  // TODO: When generated-card rendering/storage is fully automated, pass the rendered
  // durable generatedCardUrl into this plan before creating the Instagram media container.
...
function buildMetaSetupFailures(error, platform = "facebook") {
  ...
  if (
    ...
    failures.push("The saved token is not being accepted as a Live News Page/Instagram publishing token.");
    failures.push("Generate a token for the Live News Page with pages_read_engagement, pages_manage_posts, instagram_basic, and instagram_content_publish, then save it in Railway as META_PAGE_ACCESS_TOKEN.");
    failures.push("In Meta Business Settings, the app/system user or connected user must have content-creation access for the Live News Facebook Page and connected Instagram account.");
    return failures;
  }
...
async function publishInstagramDraft(draft, options = {}) {
  ...
  try {
    tokenResolution = await resolveFacebookPageAccessToken(config, options.fetchImpl);
    // TODO: Keep this media-container step behind editor approval and render readiness
    // as Instagram card rendering becomes an automated Live News service.
