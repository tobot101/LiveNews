...
function isLiveNewsHomepageUrl(value) {
  const raw = cleanText(value);
  if (!raw) return false;
  try {
    const parsed = new URL(raw);
    if (!/(^|\.)newsmorenow\.com$/i.test(parsed.hostname)) return false;
...
function validateSocialVariantSelection(draft, platformValue, variantId) {
  ...
  const rawExactArticleUrl = cleanText(variant?.exactArticleUrl || draft?.linkState?.exactArticleUrl);
  const exactArticleUrl = normalizeExactArticleUrl(rawExactArticleUrl);
  if (isLiveNewsHomepageUrl(rawExactArticleUrl)) {
    failures.push("Selected variant cannot use the Live News homepage as an article link.");
  }
  if (!exactArticleUrl) {
    warnings.push("Selected variant saved. Posting remains locked until an exact /stories/... Live News article URL exists.");
  }
  if (rawExactArticleUrl && !exactArticleUrl && !isLiveNewsHomepageUrl(rawExactArticleUrl)) {
    warnings.push("Current link is not publish-ready and will be replaced by the exact Live News story URL before posting.");
  }
...
function buildVariantSnapshot(draft, platform, variant, exactArticleUrl) {
  ...
  return {
    ...
    renderStatus: cleanText(variant?.renderStatus || variant?.imagePlan?.renderStatus || draft?.platforms?.instagram?.imagePlan?.renderStatus),
  };
}
...
function recordSocialVariantSelectionInStore(store, draft, platformValue, variantId, options = {}) {
  ...
  const record = {
    ...
    selectedVariantId: validation.normalized.variantId,
    selectedAt,
    selectedBy: cleanText(options.selectedBy || "editor"),
...
function getSelectionForDraft(store, draft, platformValue) {
  ...
  const selection = (store?.selections || []).find((item) => item.selectionKey === key) || null;
  return selection
    ? {
        ...selection,
        variantId: cleanText(selection.variantId || selection.selectedVariantId),
        selectedVariantId: cleanText(selection.selectedVariantId || selection.variantId),
      }
    : null;
}
