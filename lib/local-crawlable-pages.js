...
function isOfficialOrCivicSource(source = {}) {
  return (
    source.trustLevel === "official" ||
    source.trust_level === "official" ||
    ["official_city", "official_county", "police_fire", "school", "transit", "weather"].includes(source.sourceType || source.source_type)
  );
}

function getMeaningfulLocalSignals30d(city = {}, context = {}, options = {}) {
  const now = new Date(options.now || new Date()).getTime();
  const cutoff = now - 30 * 24 * 60 * 60 * 1000;
  return (context.inputSignals || []).filter((signal) => {
    const signalTime = new Date(signal.published_at || signal.discovered_at || signal.fetched_at || 0).getTime();
    if (!Number.isFinite(signalTime) || signalTime < cutoff) return false;
    const cityCandidates = Array.isArray(signal.city_candidates_json)
      ? signal.city_candidates_json
      : (() => {
          try {
            const parsed = JSON.parse(signal.city_candidates_json || "[]");
            return Array.isArray(parsed) ? parsed : [];
          } catch {
            return [];
          }
        })();
    const matchesCity = cityCandidates.some((candidate) => candidate.city_id === city.id && Number(candidate.confidence || 0) >= 50);
    if (!matchesCity) return false;
    return cleanText(signal.title).length >= 12 && cleanText(signal.canonical_url || signal.original_url);
  });
}
...
function getCitySeoDecision(city = {}, clusters = [], context = {}, options = {}) {
  ...
  const sourceLinks = clusters.flatMap((cluster) => getClusterSignals(cluster, context)).filter((source) => source.url);
  const meaningfulSignals30d = getMeaningfulLocalSignals30d(city, context, options);
  const hasClusterDepth = clusters.length >= 5;
  const hasSignalDepth = meaningfulSignals30d.length >= 12;
  const hasDistinctSources = sourceMix.length >= 2;
  const hasOfficialOrCivicSource =
    sourceMix.some(isOfficialOrCivicSource) ||
    sourceDirectory.some(isOfficialOrCivicSource);
  const hasUniqueCityMetadata = Boolean(city.id && city.name && city.slug && city.state_slug && city.state_abbr);
  const hasVisibleLastUpdated = clusters.length > 0 && Boolean(getLastUpdated(clusters));
  const hasSourceLinks = sourceLinks.length > 0 || sourceDirectory.some((source) => source.homepage_url);
  const reasons = [];
  if (!(hasClusterDepth || hasSignalDepth)) reasons.push("needs 5+ live clusters or 12+ meaningful 30-day signals");
  if (!hasDistinctSources) reasons.push("needs 2+ distinct sources");
  if (!hasOfficialOrCivicSource) reasons.push("needs 1+ official or civic source");
  if (!hasUniqueCityMetadata) reasons.push("needs unique city metadata");
  if (!hasVisibleLastUpdated) reasons.push("needs visible last-updated timestamp");
  if (!hasSourceLinks) reasons.push("needs visible source links");
  return {
    indexable:
      (hasClusterDepth || hasSignalDepth) &&
      hasDistinctSources &&
      hasOfficialOrCivicSource &&
      hasUniqueCityMetadata &&
    ...
    checks: {
      ...
      meaningfulSignals30d: meaningfulSignals30d.length,
      distinctSourceCount: sourceMix.length,
      officialOrCivicSourceCount: sourceMix.filter(isOfficialOrCivicSource).length + sourceDirectory.filter(isOfficialOrCivicSource).length,
      hasUniqueCityMetadata,
      hasVisibleLastUpdated,
...
function getTopicSeoDecision(city = {}, topic = "", clusters = [], context = {}) {
  const sourceMix = getSourceMix(clusters, context);
  const usefulLocalContext = Boolean(city.name && city.state_abbr && topicLabel(topic) && clusters.some((cluster) => cleanText(cluster.summary || cluster.headline)));
  const reasons = [];
  if (clusters.length < 3) reasons.push("needs 3+ live topic clusters");
  if (sourceMix.length < 2) reasons.push("needs 2+ distinct sources");
  if (!usefulLocalContext) reasons.push("needs useful local context");
  const indexable = clusters.length >= 3 && sourceMix.length >= 2 && usefulLocalContext;
  return {
    indexable,
...
function getStateSeoDecision(cities = [], citySummaries = [], context = {}, options = {}) {
  ...
  if (!indexableCityCount && allClusters.length < 5) reasons.push("needs an indexable city or 5+ live state clusters");
  if (sourceMix.length < 2) reasons.push("needs 2+ distinct sources across live state coverage");
  const indexable = Boolean(stateName) && sourceMix.length >= 2 && (indexableCityCount > 0 || allClusters.length >= 5);
  return {
    indexable,
    robots: indexable ? "index, follow" : "noindex, follow",
    reasons,
...
function getCrawlableLocalSitemapGroups(options = {}) {
  ...
  for (const [cityId, cityClusters] of cityGroups.entries()) {
    ...
    for (const topic of topics) {
      ...
      if (!getTopicSeoDecision(city, topic, topicClusters, context).indexable) continue;
      groups.topics.push({
        type: "topic",
