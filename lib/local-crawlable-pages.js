...
function LocalLiveArticleFeedPanel(city = {}) {
  const cityName = escapeHtml(city.name || "");
  const stateAbbr = escapeHtml(city.state_abbr || "");
  return `<section class="panel local-live-feed-panel" data-local-live-feed data-city-name="${cityName}" data-state-abbr="${stateAbbr}">
    <div class="section-heading">
      <div>
        <p class="story-kicker">Recent Local Articles</p>
        <h2>Latest local articles</h2>
      </div>
      <span class="section-tag">Last 7 days</span>
    </div>
    <p class="panel-note">Live News checks recent source-linked local coverage for ${cityName}, ${stateAbbr} and keeps this list limited to current public updates.</p>
    <p class="local-crawl-muted" data-local-live-feed-status>Loading recent local articles...</p>
    <div class="local-crawl-grid local-live-feed-list" data-local-live-feed-list></div>
  </section>
  <script>
    (function() {
      ...
      function renderItems(items, sourceCount) {
        ...
        items.forEach(function(item) {
          ...
          var summary = isWeakSummary(item.liveNewsSummary || item.summary) ? "" : (item.liveNewsSummary || item.summary);
          var source = item.sourceName || item.source || "Original source";
          var category = item.category || "Local";
          var published = formatTime(item.publishedAt);
          var link = item.link || item.url || "";
          card.innerHTML =
            '<div class="story-eyebrow"><span>' + escapeText(category) + '</span>' + (published ? '<span>' + escapeText(published) + '</span>' : '') + '</div>' +
            ...
            '<div class="local-card-meta"><span>' + escapeText(source) + '</span>' + (link ? '<a href="' + escapeText(link) + '" target="_blank" rel="noopener noreferrer">Read original source</a>' : '') + '</div>';
      ...
      fetch("/api/local?" + params.toString(), { headers: { "Accept": "application/json" } })
        ...
        .catch(function() {
          if (status) status.textContent = "Recent local articles are unavailable right now. Please try again shortly.";
        });
    })();
  </script>`;
}
...
function renderLocalCityPage(stateSlug, citySlug, options = {}) {
  ...
  ${LocalLiveArticleFeedPanel(city)}
  <section class="panel">
    <h2>Local Pulse</h2>
