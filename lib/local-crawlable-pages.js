...
<body class="local-theme">
  ...
  <script>
    document.addEventListener("click", function(event) {
      ...
      if (saveCity) {
        try { localStorage.setItem("ln_local_place", saveCity.getAttribute("data-save-city") || ""); } catch (error) {}
        saveCity.textContent = "City saved";
      }
      var followTopic = event.target.closest("[data-follow-topic]");
      if (followTopic) {
        try {
          var existing = JSON.parse(localStorage.getItem("ln_followed_topics") || "[]");
          var topic = followTopic.getAttribute("data-follow-topic");
          if (topic && existing.indexOf(topic) === -1) existing.push(topic);
          localStorage.setItem("ln_followed_topics", JSON.stringify(existing));
        } catch (error) {}
...
function renderLocalCityPage(stateSlug, citySlug, options = {}) {
  ...
  const body = `<section class="panel local-crawl-hero">
    ...
    <div class="local-crawl-actions">
      <button class="btn" data-save-city="${escapeHtml(JSON.stringify({ name: city.name, state: city.state_abbr, display: `${city.name}, ${city.state_abbr}` }))}">Save city</button>
...
function renderLocalTopicPage(stateSlug, citySlug, topicSlug, options = {}) {
  ...
  const body = `<section class="panel local-crawl-hero">
    ...
    <div class="local-crawl-actions">
      <button class="btn" data-save-city="${escapeHtml(JSON.stringify({ name: city.name, state: city.state_abbr, display: `${city.name}, ${city.state_abbr}` }))}">Save city</button>
      <button class="btn ghost" data-follow-topic="${escapeHtml(matchingTopic)}">Follow topic</button>
      <button class="btn ghost" disabled>Newsletter placeholder</button>
    </div>
  </section>
  <section class="panel">
