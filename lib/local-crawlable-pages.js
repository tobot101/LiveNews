...
function renderLocalCitiesPage(options = {}) {
  ...
  const cities = [...(context.cities || [])]
    .filter((city) => city.name && city.slug && city.state_slug)
    .sort((left, right) => {
      const stateSort = cleanText(left.state_name).localeCompare(cleanText(right.state_name));
      return stateSort || cleanText(left.name).localeCompare(cleanText(right.name));
    });
  const popularCities = [...cities]
    .sort((left, right) => Number(right.population || 0) - Number(left.population || 0) || left.name.localeCompare(right.name))
    .slice(0, 12);
  const grouped = new Map();
  for (const city of cities) {
    const key = city.state_name || city.state_abbr || "Other";
    const list = grouped.get(key) || [];
    list.push(city);
    grouped.set(key, list);
  }
  const cityLink = (city) => `<a class="local-directory-city-link" href="${escapeHtml(localCityPath(city))}" data-city-name="${escapeHtml(city.name.toLowerCase())}" data-state-abbr="${escapeHtml(city.state_abbr.toLowerCase())}" data-state-name="${escapeHtml(city.state_name.toLowerCase())}">
  ...
  const body = `<section class="panel local-city-directory-hero">
    ...
    <p class="panel-note">Choose a city to see live local updates from the last 7 days.</p>
  ...
  <section class="panel">
    ...
    <p class="panel-note">Cities are grouped by state. Select a city to view its crawlable Live News local dashboard.</p>
    <div class="local-directory-states">${[...grouped.entries()].map(([stateName, stateCities]) => `
      <section class="local-directory-state" data-state-group="${escapeHtml(stateName.toLowerCase())}">
        <h3>${escapeHtml(stateName)}</h3>
        <div class="local-directory-grid">${stateCities.map(cityLink).join("")}</div>
      </section>`).join("")}</div>
  ...
  <script>
    (function() {
      ...
      var links = Array.prototype.slice.call(document.querySelectorAll(".local-directory-city-link"));
      var stateGroups = Array.prototype.slice.call(document.querySelectorAll(".local-directory-state"));
      function filterCities() {
        var query = String(input && input.value || "").trim().toLowerCase();
        var visible = 0;
        links.forEach(function(link) {
          var haystack = [
            link.getAttribute("data-city-name"),
            link.getAttribute("data-state-abbr"),
            link.getAttribute("data-state-name")
          ].join(" ");
        ...
        stateGroups.forEach(function(group) {
          group.hidden = !group.querySelector(".local-directory-city-link:not([hidden])");
        });
        if (empty) empty.hidden = visible !== 0;
      }
      if (input) input.addEventListener("input", filterCities);
  ...
  return {
    ...
    html: pageShell({
      ...
      description: "Choose a city to see Live News local updates from the last 7 days, grouped by state with source-linked city pages.",
