...
function classifyEntertainmentStory(story = {}) {
  ...
  const reasons = [];
  const existingSubbeat = normalizeText(story.entertainmentSubbeat || story.entertainmentClassification?.subbeat);
  const existingConfidence = Number(story.entertainmentConfidence ?? story.entertainmentClassification?.confidence ?? 0);
  ...
  const hasApprovedEntertainmentSignal =
    ALLOWED_SUBBEATS.has(existingSubbeat) ||
    story.entertainmentClassification?.isEntertainment === true ||
    existingConfidence >= 45;
  ...
  if (hasApprovedEntertainmentSignal) reasons.push("approved_entertainment_classification");
  ...
  const hasCentralEntertainmentSignal =
    ...
    hasApprovedEntertainmentSignal ||
  ...
  const subbeat = ALLOWED_SUBBEATS.has(existingSubbeat)
    ? existingSubbeat
    : ALLOWED_SUBBEATS.has(bestSubbeat?.subbeat)
    ? bestSubbeat.subbeat
    : "general_entertainment";
  ...
  if (hasApprovedEntertainmentSignal) confidence += Math.max(18, Math.min(existingConfidence, 30));
