...
const WEAK_WRITING_PATTERNS = [
  ...
  { id: "drama_alert", pattern: /\bdrama alert\b/i, phrase: "Drama alert" },
  { id: "bombshell", pattern: /\bbombshell\b/i, phrase: "Bombshell" },
  { id: "spills_tea", pattern: /\bspills? tea\b/i, phrase: "Spills tea" },
  { id: "fans_are_saying", pattern: /\bfans are saying\b/i, phrase: "Fans are saying" },
  { id: "secret_romance", pattern: /\bsecret romance\b/i, phrase: "Secret romance" },
  { id: "love_triangle", pattern: /\blove triangle\b/i, phrase: "Love triangle" },
  { id: "caught_cheating", pattern: /\bcaught cheating\b/i, phrase: "Caught cheating" },
];
...
function buildArticleWritingContext(story = {}) {
  ...
  const summaryValues = uniqueClean([
    story.sourceFacts,
    story.facts?.map((fact) => fact.text || fact),
    story.sourceSummary,
    story.rawSummary,
    story.rssDescription,
    story.liveNewsDek,
...
function copyRiskTeacher(candidate, context) {
  const text = getCandidateText(candidate);
  const sourceTexts = uniqueClean(context.doNotSay || []);
  const risks = sourceTexts
    .map((sourceText) => detectCopyRisk(text, sourceText))
    .filter((risk) => risk.risk);
  const distance = explainCopyRisk(text, sourceTexts, {
    ...context,
    doNotCopyPhrases: sourceTexts,
  });
  const distanceBlocking = distance.risk === "blocked" || distance.risk === "high";
  const blocking = risks.some((risk) => risk.blocking) || distanceBlocking;
  const score = Math.min(
    risks.length ? Math.min(...risks.map((risk) => risk.score)) : 100,
    distance.score
  );
  return makeTeacher(
    "CopyRiskTeacher",
    !blocking,
    score,
    risks.length || distance.risk !== "low"
      ? [...risks.map((risk) => risk.reason), distance.explanation].filter(Boolean).join(" ")
      : "Writing does not closely copy blocked publisher/source wording.",
    blocking,
    { risks, copyDistance: distance }
  );
}
...
function getSummaryTextForWriting(story = {}) {
  return uniqueClean([
    story.summary,
    story.summaryShort,
    story.summaryLong,
    story.liveNewsSummary,
    story.liveNewsSummaryLong,
  ]).join(" ");
}
...
function buildFieldGate(fieldName, text, context) {
  ...
  return getWritingQualityGateResult(clean, context, fieldName);
}

function summarizeFieldExams(fieldGates) {
  ...
  const fieldTotals = entries.reduce((result, [fieldName, gate]) => {
    result[fieldName] = {
  ...
  return {
    schemaVersion: "live-news-writing-exam-summary-v1",
...
function buildStoryWritingPackage(story = {}) {
  const sourceBackedSummary = uniqueClean([
    story.sourceSummary,
    story.rawSummary,
    story.rssDescription,
    story.sourceFacts,
  ...
  const context = buildArticleWritingContext(sourceBackedSummary ? {
    ...story,
    description: "",
    metaDescription: "",
  ...
  const descriptionGeneration = generateDescriptionCandidates(context);
  const rawSummaryText = getSummaryTextForWriting(story);
  const title = cleanText(story.title || story.headline || story.liveNewsHeadline || "");
  const descriptionSelection = selectBestWritingCandidate(
    [
      { id: "existingDescription", label: "Existing description", text: story.description || story.metaDescription || "" },
      { id: "dek", label: "Dek", text: story.dek || story.liveNewsDek || "" },
      { id: "summary", label: "Summary", text: rawSummaryText },
      ...normalizeCandidateList(descriptionGeneration),
    ].filter((candidate) => cleanText(candidate.text)),
    context,
    "description"
  );
  const selectedDescription = descriptionSelection.selected?.evaluation?.passed
    ? descriptionSelection.selected.text
    : cleanText(story.description || story.metaDescription || story.dek || story.summaryShort || rawSummaryText || "");
  const fields = {
    title,
    description: selectedDescription,
    dek: selectedDescription || cleanText(story.dek || story.liveNewsDek || ""),
    summary: selectedDescription || rawSummaryText,
    whyItMatters: cleanText(story.whyItMatters || story.liveNewsWhyItMatters || ""),
    metaTitle: cleanText(story.metaTitle || (title ? `${title} | Live News` : "")),
    metaDescription: cleanText(story.metaDescription || selectedDescription),
  };
  const fieldGates = Object.fromEntries(
    Object.entries(fields).map(([fieldName, text]) => [fieldName, buildFieldGate(fieldName, text, context)])
  );
  const writingExam = summarizeFieldExams(fieldGates);
  const exactUrlBlocked = (context.missingContext || []).some((entry) =>
    ["exact_article_url_missing", "homepage_url_blocked"].includes(entry)
  );
  const weakContext = (context.missingContext || []).length > 0 || descriptionGeneration.status === "needs_more_context";
  const writingQualityStatus = writingExam.passed
    ? "ready"
    : exactUrlBlocked || writingExam.blockingReasons.length
      ? "blocked"
      : weakContext
        ? "needs_review"
        : "needs_review";
  ...
  return {
    schemaVersion: "live-news-writing-package-v1",
    ...fields,
    context,
    descriptionGeneration,
    descriptionCandidates: descriptionSelection.candidates,
    fieldGates,
    writingExam,
    ...
    blockingReasons: writingExam.blockingReasons,
