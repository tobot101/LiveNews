...
const { buildSeoPackage } = require("./seo-package");
const { buildOriginalStoryWritingPackage } = require("./original-writing-package");
const { recordApprovedWritingEdit } = require("./writing-memory");
...
function getApprovedStorySummaries(stories = listApprovedStories()) {
  return summarizeApprovedStories(stories);
}

function summarizeApprovedStories(stories) {
...
function getCandidateStoryIds(item) {
  const ids = [item.storyId, item.id, item.slug].map(cleanText).filter(Boolean);
...
function enrichNewsItemsWithApprovedStories(items, approvedStories = listApprovedStories()) {
  return (items || []).map((item) => {
    ...
    return {
      ...
      liveNewsDek: story.dek,
      liveNewsSummaryLong: story.summaryLong,
      liveNewsKeyPoints: story.keyPoints || [],
      liveNewsWhyItMatters: story.whyItMatters,
      liveNewsStoryId: story.storyId,
      description: story.description,
      metaDescription: story.metaDescription,
      writingQualityStatus: story.writingQualityStatus,
      writingExam: story.writingExam,
      teacherChecks: story.teacherChecks,
      factMap: story.factMap,
      writerRoom: story.writerRoom,
      copyRisk: story.copyRisk,
      rewriteSession: story.rewriteSession,
      quality: story.quality,
      originalPublisherTitle: story.originalPublisherTitle || item.originalPublisherTitle,
      thumbnailUrl: story.thumbnailUrl || item.thumbnailUrl,
      imageCredit: story.imageCredit || item.imageCredit,
      reviewStatus: "approved",
    };
  });
}

function enrichNewsPayloadWithApprovedStories(payload, approvedStories = listApprovedStories()) {
  const enrichedTopStoryOfDay = enrichNewsItemsWithApprovedStories(
    [payload.topStoryOfDay].filter(Boolean),
    approvedStories
  )[0] || null;
  const enrichedTopStoryOfWeek = enrichNewsItemsWithApprovedStories(
    [payload.topStoryOfWeek].filter(Boolean),
    approvedStories
  )[0] || null;
  return {
    ...payload,
    approvedStories: summarizeApprovedStories(approvedStories),
    topStoryOfDay: enrichedTopStoryOfDay,
    topStoryOfWeek: enrichedTopStoryOfWeek,
...
function validateDraftForApproval(draft) {
  ...
  const writingPackage = draft ? buildOriginalStoryWritingPackage(draft) : null;
  if (!writingPackage || writingPackage.writingQualityStatus !== "ready") {
    failures.push(
      `Draft writing quality is not ready: ${writingPackage?.writingQualityStatus || "blocked"}. ${
        (writingPackage?.blockingReasons || writingPackage?.missingContext || []).slice(0, 4).join(" ")
      }`.trim()
    );
  }
  return { ok: failures.length === 0, failures, writingPackage };
}
...
function toApprovedStory(draft, options = {}) {
  ...
  if (writingPackage.writingQualityStatus !== "ready") {
    const error = new Error(`Draft writing quality is not ready: ${writingPackage.writingQualityStatus}.`);
    error.failures = writingPackage.blockingReasons || writingPackage.missingContext || [];
    error.writingPackage = writingPackage;
    throw error;
  }
  const summary = buildSummaryText({
    ...draft,
    summary: writingPackage.summary ? [writingPackage.summary] : draft.summary,
  });
 ...
 const provisionalStory = {
    ...
    title: writingPackage.title || cleanText(draft.headline),
    headline: writingPackage.title || cleanText(draft.headline),
    description: writingPackage.description || truncate(summary.slice(0, 2).join(" "), 240),
    dek: writingPackage.dek || cleanText(draft.dek),
    summary,
    summaryText: writingPackage.summary || summary.join(" "),
    summaryShort: truncate(writingPackage.description || summary.slice(0, 2).join(" "), 240),
    summaryLong: writingPackage.summary || summary.join(" "),
    keyPoints: (draft.keyPoints || []).map(cleanText).filter(Boolean).slice(0, 5),
    whyItMatters: writingPackage.whyItMatters || cleanText(draft.whyItMatters),
    ...
    originalSourceUrl,
    thumbnailUrl: cleanText(draft.thumbnailUrl || ""),
    imageCredit: cleanText(draft.imageCredit || draft.primarySourceName || ""),
    supportingSources,
    originalPublisherTitle: cleanText(draft.originalPublisherTitle || ""),
    metaTitle: truncate(writingPackage.metaTitle || `${cleanText(draft.headline)} | Live News`, 65),
    metaDescription: truncate(writingPackage.metaDescription || buildMetaDescription(draft), 155),
  ...
  const seoPackage = buildSeoPackage(provisionalStory);
  return {
    ...provisionalStory,
    canonicalUrl: seoPackage.canonicalUrl || provisionalStory.canonicalUrl,
    metaTitle: seoPackage.seoTitle || provisionalStory.metaTitle,
    metaDescription: seoPackage.metaDescription || provisionalStory.metaDescription,
    schemaType: seoPackage.structuredDataType || provisionalStory.schemaType,
    seoPackage,
    quality: {
      ...provisionalStory.quality,
      seoPackage,
    },
  };
}

function getDraftFieldText(draft, fieldName) {
  const fieldMap = {
    title: draft.headline || draft.title,
    description: draft.description,
    dek: draft.dek,
    summary: buildSummaryText(draft).join(" "),
    whyItMatters: draft.whyItMatters,
    seoTitle: draft.metaTitle,
    metaDescription: draft.metaDescription,
    homepageCard: draft.homepageCardText || draft.description,
  };
  return cleanText(fieldMap[fieldName] || draft[fieldName] || "");
}

function getApprovedFieldText(story, fieldName) {
  const fieldMap = {
    title: story.title || story.headline,
    description: story.description,
    dek: story.dek,
    summary: story.summaryText || story.summaryLong || (story.summary || []).join(" "),
    whyItMatters: story.whyItMatters,
    seoTitle: story.metaTitle,
    metaDescription: story.metaDescription,
    homepageCard: story.summaryShort || story.description,
  };
  return cleanText(fieldMap[fieldName] || story[fieldName] || "");
}

function recordApprovalWritingMemory(draft, story, options = {}) {
  const edits = Array.isArray(options.writingEdits) ? options.writingEdits : [];
  if (!edits.length) return [];

  return edits.map((edit) => {
    const fieldName = cleanText(edit.fieldName || "description");
    return recordApprovedWritingEdit(
      {
        storyId: story.storyId,
        category: story.category,
        fieldName,
        weakOutput: edit.weakOutput || getDraftFieldText(draft, fieldName),
        approvedOutput: edit.approvedOutput || getApprovedFieldText(story, fieldName),
        editorReason: edit.editorReason || edit.reason || "Editor approved a stronger Live News wording.",
        lesson: edit.lesson,
        writingShape: edit.writingShape,
        sourceSafetyNotes: edit.sourceSafetyNotes,
        publisherText: edit.publisherText || draft.originalPublisherTitle,
        createdAt: edit.createdAt || story.approvedAt,
      },
      {
        storePath: options.writingMemoryStorePath,
        limit: options.writingMemoryLimit,
      }
    );
  });
}
...
function approveDraft(draft, options = {}) {
  ...
  const writingMemoryResults = recordApprovalWritingMemory(draft, story, options);
  const savedWritingLessons = writingMemoryResults
    .filter((result) => result.ok && result.record?.lesson)
    .map((result) => result.record.lesson);
  if (savedWritingLessons.length) {
    story.writingMemoryLessonsUsed = uniqueBy(
      [...(story.writingMemoryLessonsUsed || []), ...savedWritingLessons],
      (lesson) => lesson.toLowerCase()
    );
    story.quality = {
      ...(story.quality || {}),
      writingMemoryResults,
    };
  }
