...
function buildDek(packet, style) {
  ...
  if (style.openingType === "source_cluster" && count > 1) {
    return `This ${category.toLowerCase()} draft is built from ${count} source entries, led by ${source}, and waits for editor review.`;
  }
  ...
  if (style.openingType === "timeline_first" && packet.sourcePublishedAt) {
    return `The latest source timestamp is ${packet.sourcePublishedAt}; public posting stays paused until review checks pass.`;
  }
  return `A review-only brief prepared from the source packet led by ${source}.`;
}
...
function buildSummary(packet, style) {
  ...
  const lines = {
    direct_fact: [
      `${focus} is a ${category} story held for editor review before publication.`,
      sourceContext,
    ],
    source_cluster: [
      `The available coverage around ${focus} is grouped into a source packet before drafting.`,
      sourceContext,
    ],
    reader_impact: [
