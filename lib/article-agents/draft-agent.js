...
function buildDek(packet, style) {
  ...
  if (style.openingType === "source_cluster" && count > 1) {
    return `Live News follows ${focus} through ${count} attributed updates led by ${source}.`;
  }
  if (style.openingType === "reader_impact") {
    return `${focus} is tracked with clear context from ${source} for readers following this ${category.toLowerCase()} story.`;
  }
...
function buildSummary(packet, style) {
  ...
  const lines = {
    direct_fact: [
      `${focus} is a ${category} story summarized with visible attribution.`,
      sourceContext,
    ],
    source_cluster: [
    ...
    timeline_first: [
      `The coverage record includes the latest source timestamp and attributed facts.`,
