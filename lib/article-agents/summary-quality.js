...
const FILLER_PATTERNS = [
  ...
  /\bthe account includes\b/i,
  /\bthe available detail includes\b/i,
  /\bthe .* angle includes\b/i,
  /\bthe .* detail includes\b/i,
  /\bsource details include\b/i,
...
const GRAMMAR_GUARD_PATTERNS = [
  /\b(?:has|have|had)\s+says\b/i,
  /\bhas\s+say\b/i,
  /\b(?:is|are|was|were)\s+(?:is|are|was|were)\b/i,
  /\bto\s+to\b/i,
  /\b\d+\s+to\d+\b/i,
  /\b(?:clarification|approval|permission|answers|support|funding)\s+from\.$/i,
  /,\s*\.$/,
];
...
const DANGLING_END_PATTERNS = [
  ...
  /\b[A-Z][A-Z]+,\s+[A-Z][A-Za-z .'-]+\s+--\b/,
  /\.\s+[A-Z][A-Z]+,\s+[A-Z][A-Za-z .'-]+\s+--\b/,
  /--\s*$/,
];
...
function flattenSummaryResearchEvidence(research) {
  const values = [];
  if (!research || typeof research !== "object") return values;
  values.push(...(Array.isArray(research.facts) ? research.facts : []));
  values.push(research.evidenceText);
  values.push(research.sourceMetadata?.description);
  values.push(research.sourceMetadata?.title);
  return values;
}

function getSourceEvidenceText(item) {
  return [
    item?.summary,
    item?.sourceSummary,
    ...flattenSummaryResearchEvidence(item?.summaryResearch),
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ");
...
function getUsefulDetailTokens(item) {
  ...
  return tokenize(getSourceEvidenceText(item))
...
function hasLocalTitleBackedDetail(item, summary) {
  const category = String(item?.category || "").toLowerCase();
  if (category !== "local") return false;
  const textTokens = new Set(tokenize(summary));
  const titleHits = tokenize(item?.title || "").filter((token) => textTokens.has(token)).length;
  return titleHits >= 3 && wordCount(summary) >= MIN_SUMMARY_WORDS;
}

function getSentenceIntegrityFailures(summary) {
  const clean = cleanText(summary);
  if (!clean || clean === FALLBACK_SUMMARY) return [];
  const failures = [];
  const sentences = splitSentences(clean);
  if (!sentences.length) failures.push("no_complete_sentence");

  for (const sentence of sentences) {
    const words = getRepeatWords(sentence);
    const last = words[words.length - 1] || "";
    if (DANGLING_END_WORDS.has(last)) failures.push("dangling_sentence_end");
    if (DANGLING_END_PATTERNS.some((pattern) => pattern.test(sentence))) {
      failures.push("fragmented_source_sentence");
    }
  }

  if (/[,;:]\s*$/.test(clean)) failures.push("dangling_punctuation");
  if (DANGLING_END_PATTERNS.some((pattern) => pattern.test(clean))) {
    failures.push("fragmented_source_sentence");
  }
...
function evaluateSummaryQuality(item, summary, options = {}) {
  ...
  if (!isFallback && GRAMMAR_GUARD_PATTERNS.some((pattern) => pattern.test(clean))) failures.push("grammar_guard");
  if (!isFallback) failures.push(...getSentenceIntegrityFailures(clean));
  if (
    !isFallback &&
  ...
  if (!isFallback && !hasFeedBackedDetail(item, clean) && !hasLocalTitleBackedDetail(item, clean)) {
    failures.push("missing_feed_detail");
  }
  ...
  return {
    ...
    metrics: {
      ...
      evidenceSimilarity: getTokenSimilarity(getSourceEvidenceText(item), clean),
      sentenceIntegrityFailures: getSentenceIntegrityFailures(clean),
