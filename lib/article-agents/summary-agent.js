...
const SUMMARY_AGENT_VERSION = "live-news-summary-agent-v4";
...
const UNSAFE_TRIM_END_WORDS = new Set([
  "a",
  "an",
  "and",
  "as",
...
const UNSAFE_TRIM_END_PATTERNS = [
  /\b(?:may|might|could|would|will|can|should)\s+have\s+(?:fallen|gone|been|taken)$/i,
  /\b(?:accused|charged|convicted|involved|participating|seeking|trying|working|aimed)$/i,
  /\b(?:opened|closed|blocked|launched|announced|reported|said)\s+(?:the|a|an)$/i,
];
...
const TITLE_PATTERNS = [
  ...
  [/^(.+?) in (.+?) County hits (100%) containment by (.+)$/i, "$1 reached full containment by $4 in the county"],
  [/^(.+?) hits (100%) containment by (.+)$/i, "$1 reached full containment by $3"],
  [/^(.+?) in (.+?) County remains 100% contained by (.+)$/i, "$1 remained fully contained by $3 in the county"],
  [/^(.+?) remains 100% contained by (.+)$/i, "$1 remained fully contained by $2"],
  [/^(.+?) Is in [‘']Critical Condition[’'] in Florida Hospital$/i, "$1 was reported in critical condition at a Florida hospital"],
  [/^(.+?) Is in [‘']Critical Condition[’'] in (.+)$/i, "$1 was reported in critical condition at $2"],
  [/^Spokesman says (.+?) hospitalized in critical condition$/i, "$1 was hospitalized in critical condition, a spokesman says"],
  [/^Former (.+?) hospitalized in critical condition, spokesman says$/i, "Former $1 was hospitalized in critical condition, a spokesman says"],
  [/^Cal Fire responds to (.+?) fires along the highway in (.+)$/i, "Cal Fire responded to $1 fires along a highway in $2"],
  [/^(.+?) suffers (.+?) after colliding with (.+)$/i, "$1 suffered $2 after a collision with $3"],
  [/^(.+?) grabs (.+?) as (.+?) draws with (.+)$/i, "$1 recorded $2 as $3 drew with $4"],
  [/^(.+?) draws with (.+)$/i, "$1 drew with $2"],
  [/^(.+?) faces (.+?) in (.+?) derby as busy schedule$/i, "$1 faces $2 in a $3 derby during a busy schedule"],
  [/^(.+?) Place (.+?) in Injured List in Surprise Move$/i, "$1 placed $2 on the injured list in a surprise roster move"],
  [/^(.+?) Place (.+?) on Injured List in Surprise Move$/i, "$1 placed $2 on the injured list in a surprise roster move"],
  [/^San Diego staving off sweep for first time since (.+)$/i, "San Diego is trying to avoid a sweep for the first time since $1"],
  [/^(\d+%) containment reached for (.+?) in (.+?) County$/i, "$2 had $1 containment in a local update"],
  [/^(\d+%) containment reached for (.+)$/i, "$2 had $1 containment in a local update"],
  [/^Scripps Ranch @ San Diego Baseball Game Info$/i, "Scripps Ranch and San Diego have a baseball matchup listed"],
  [/^What's happening with (.+?) in (.+?) in (\d{4})$/i, "$1 in $2 are the focus for $3"],
  [/^(.+?) is this first-time (.+?) filmmaker’s chance to say something to help other veterans$/i, "A first-time $2 filmmaker is using $1 to address veterans' experiences"],
  [/^(.+?) is this first-time (.+?) filmmaker's chance to say something to help other veterans$/i, "A first-time $2 filmmaker is using $1 to address veterans' experiences"],
  [/^(.+?) landmark (.+?) is a disaster waiting to happen$/i, "$1 landmark $2 is facing disaster concerns"],
  [/^Spectators gather as (.+?) returns to (.+)$/i, "$1 returned to $2 as spectators gathered"],
  [/^Why getting more (.+?) students into top UCs carries a big cost to taxpayers$/i, "Getting more $1 students into top UC campuses could raise taxpayer costs"],
  [/^Air quality alert active for (.+?) until (.+)$/i, "An air quality alert remains active for $1 until $2"],
  [/^Multiple vegetation fires ignite in (.+)$/i, "Multiple vegetation fires ignited in $1"],
  [/^Agencies discuss what to cut as state deals with \$(\d+) billion deficit$/i, "State agencies discussed possible cuts during a $$$1 billion deficit"],
  [/^Could (.+?) finally bring stability to (.+?)$/i, "$1 could bring stability to $2"],
  [/^(.+?) unemployment decreases to ([\d.]+%) in (.+)$/i, "$1 unemployment decreased to $2 in $3"],
  [/^Fewer hours, services anticipated at (.+?) public libraries under proposed city budget$/i, "$1 public libraries may face fewer hours and services under the proposed city budget"],
  [/^Two years later, (.+?) rebuild timeline stretches on$/i, "$1 rebuild timeline is stretching two years later"],
  [/^(.+?) star centerfielder joins Humane Society’s annual Walk for Animals$/i, "A $1 centerfielder joined the Humane Society's annual Walk for Animals"],
  [/^The lost restaurants of (.+?): Before the counter culture$/i, "A $1 restaurant history piece looks back before the counter-culture era"],
  [/^Robot dances at the airport gate, then delays (.+?) flight from (.+?) to (.+)$/i, "A robot at an airport gate delayed a $1 flight from $2 to $3"],
  [/^Dancing robot delays (.+?) flight after (.+?) confiscates its oversized batteries$/i, "A dancing robot delayed a $1 flight after $2 confiscated its oversized batteries"],
  [/^(.+?) Lineup vs (.+?): (.+?) Out$/i, "$3 is out of the $1 lineup against $2"],
  [/^(.+?), creator of (.+?), is laying off workers in (.+?) and across (.+)$/i, "$1 is laying off workers in $3 and across $4"],
  [/^(.+?) defeat (.+?), ([\d-]+), at (.+)$/i, "$1 defeated $2 $3 at $4"],
  [/^(.+?)'s Celebration of Champions unites families affected by (.+)$/i, "$1's Celebration of Champions brought together families affected by $2"],
  [/^Volunteers clean up (.+?) along (.+)$/i, "Volunteers cleaned up $1 along $2"],
  [/^(.+?) council candidates clash over (.+)$/i, "$1 council candidates clashed over $2"],
  [/^Push to restrict corporate homeownership hits bumps$/i, "A push to restrict corporate homeownership hit bumps"],
  [/^(.+?) Shuffle Rotation Hinting at Major Roster Move$/i, "$1 shuffled the rotation while hinting at a major roster move"],
  [/^(.+?) Fire burns (.+?) in (.+?) County$/i, "$1 Fire burned $2 in the county"],
  [/^(.+?) Fire burns (.+?) in (.+)$/i, "$1 Fire burned $2 in $3"],
  [/^(.+?) spreads to (.+?) in (.+?), containment still at (\d+%)$/i, "$1 grew to $2 while containment remained at $4"],
  [/^(.+?) Fire in (.+?) - (.+?) status$/i, "$1 Fire in $2 had a $3 status update"],
  [/^(.+?) Fire in (.+?) County$/i, "$1 Fire had a status update in $2 County"],
  [/^More than a dozen small fires break out in (.+)$/i, "More than a dozen small fires broke out in $1"],
  [/^Water Molecules Found To Actively Drive Gene Transcription ProcessUniversity of California – San Diego$/i, "University of California San Diego research found water molecules actively drive gene transcription"],
  [/^'Wait Wait' for (.+?): Live in (.+?) with (.+)$/i, "Wait Wait recorded a live $2 show with $3"],
  [/^Humanoid robot named (.+?) causes (.+?) flight delay out of (.+)$/i, "A humanoid robot named $1 caused a $2 flight delay out of $3"],
  [/^Real Estate Market Trends in (.+?): Prices Fall$/i, "Real estate prices fell in $1"],
  [/^(.+?) announce agreement to transfer ownership to (.+?) group$/i, "$1 announced an ownership-transfer agreement with the $2 group"],
  [/^(.+?) announce agreement to sell team to investor group led by (.+)$/i, "$1 announced an agreement to sell the team to an investor group led by $2"],
  [/^El Cajon man gets (\d+) years for sex trafficking 15-year-old girl in California and Arizona$/i, "A man from El Cajon received a $1-year sentence in a sex-trafficking case involving a 15-year-old girl"],
  [/^El Cajon man gets (\d+) years for sex trafficking (.+)$/i, "A man from El Cajon received a $1-year sentence in a sex-trafficking case"],
  [/^Runaway Waymo that dropped passenger at (.+?) airport.+$/i, "A Waymo dropped a passenger at a California airport during a mistaken ride"],
  [/^San Diego State University does not track amount of unhoused students on campus$/i, "San Diego State University does not track how many unhoused students are on campus"],
  [/^Sacramento Report: California Coastal Commission [‘']Untouchable[’'] No More$/i, "California Coastal Commission power is under new scrutiny in a Sacramento report"],
  [/^Victims identified in recent (.+?) traffic fatalities$/i, "Victims were identified after recent $1 traffic fatalities"],
  [/^Assembly leaders rewrite (.+?) plan to redefine role of state schools superintendent$/i, "Assembly leaders rewrote $1 plan for the state schools superintendent's role"],
  [/^State public health officials file complaint against (.+?) business in (.+?) area$/i, "State public health officials filed a complaint against a $1 business in the $2 area"],
  [/^Beam Global \(NASDAQ: BEEM\) plans early end to (.+?) lease$/i, "Beam Global plans an early end to its $1 lease"],
  [/^How many homes are getting built in (.+?) in (\d{4})$/i, "New home construction in $1 is being counted for $2"],
  [/^Breaking: (.+?) Fire reported in (.+?) County on (.+)$/i, "$1 Fire had a $3 report for $2 County"],
  [/^Breaking: (.+?) Fire reported in (.+?) on (.+)$/i, "$1 Fire was reported in $2 on $3"],
  [/^(.+?) Fire reported in (.+?) County on (.+)$/i, "$1 Fire had a $3 report for $2 County"],
  [/^This is your once-in-a-generation chance to step inside this (.+?) landmark$/i, "A $1 landmark is opening for a rare chance to step inside"],
  [/^Good Morning (.+?): (.+?) in danger of being swept after loss to (.+)$/i, "$2 are in danger of being swept after a loss to $3"],
  [/^(.+?) Ride Six-Run Second and (.+?) to ([\d-]+) Win in (.+)$/i, "$1 beat $4 $3 after a six-run second"],
  [/^New Route from (.+?) to (.+?) What This Means for Travelers.+$/i, "A new $1-to-$2 route is expanding travel links"],
  [/^(.+?) Fire reported in (.+)$/i, "$1 Fire was reported in $2"],
  [/^(.+?) puts law enforcement on rooftops in the name of fundraising$/i, "$1 put law enforcement on rooftops for a fundraiser"],
  [/^(.+?) – (\d+)-Year-Old Man Killed in Bus Accident on (.+)$/i, "A $2-year-old man was killed in a bus accident on $3"],
  [/^(.+?) firefighters respond to (.+)$/i, "$1 firefighters responded to $2"],
  [/^(.+?) police investigate (.+)$/i, "$1 police investigated $2"],
  [/^(.+?) public libraries extend (.+)$/i, "$1 public libraries extended $2"],
  [/^(.+?) subway delays continue after (.+)$/i, "$1 subway delays continued after $2"],
  [/^(.+?) transit delays continue after (.+)$/i, "$1 transit delays continued after $2"],
  [/^(.+?) council approves (.+)$/i, "$1 council approved $2"],
  [/^(.+?) council rejects (.+)$/i, "$1 council rejected $2"],
  [/^(.+?) council delays (.+)$/i, "$1 council delayed $2"],
  [/^(.+?) school board approves (.+)$/i, "$1 school board approved $2"],
  [/^(.+?) school board delays (.+)$/i, "$1 school board delayed $2"],
  [/^(.+?) airport adds (.+)$/i, "$1 airport added $2"],
  [/^(.+?) airport delays (.+)$/i, "$1 airport delayed $2"],
  [/^(.+?) residents face (.+)$/i, "$1 residents faced $2"],
  [/^(.+?) businesses face (.+)$/i, "$1 businesses faced $2"],
  [/^(.+?) workers face (.+)$/i, "$1 workers faced $2"],
  [/^FAA says (.+?) flight landing at (.+?) strikes (.+)$/i, "A $1 flight landing at $2 struck $3, FAA says"],
  [/^FAA says (.+?) flight landing at (.+?) struck (.+)$/i, "A $1 flight landing at $2 struck $3, FAA says"],
  [/^(.+?) flight landing at (.+?) strikes (.+)$/i, "A $1 flight landing at $2 struck $3"],
  [/^(.+?) strikes (.+)$/i, "$1 struck $2"],
  [/^(.+?) stuns in (.+?) during (.+)$/i, "$1 appeared in $2 during $3"],
  [/^(\d+) Arrested at (.+)$/i, "$1 people were arrested at $2"],
  [/^Woman killed in (.+?) identified by medical examiner$/i, "A woman killed in $1 was identified by a medical examiner"],
  [/^Woman killed in (.+?) identified by (.+)$/i, "A woman killed in $1 was identified by $2"],
  [/^(.+?) killed in (.+?) identified by (.+)$/i, "$1 killed in $2 was identified by $3"],
  [/^Man Found Dead Along Fence Line At (.+)$/i, "A man was found dead along a fence line at $1"],
  [/^Man Found Dead Along (.+)$/i, "A man was found dead along $1"],
  [/^Body found near Chicago Executive Airport in .+? after possible car crash: police$/i, "A body was found near Chicago Executive Airport after a possible crash, police say"],
  [/^Body found near (.+?) after possible (.+?): police$/i, "A body was found near $1 after a possible $2"],
  [/^Police say body found near Chicago Executive Airport.+?after possible car crash$/i, "A body was found near Chicago Executive Airport after a possible crash, police say"],
  [/^Police say body found near (.+?) after possible (.+)$/i, "A body was found near $1 after a possible $2, police say"],
  [/^Busch stars as the surging Cubs beat the Diamondbacks 8-4 for their 5th consecutive win$/i, "Busch starred as the Cubs beat the Diamondbacks 8-4 for a fifth consecutive win"],
  [/^(.+?) beat (.+?) ([\d-]+) for (.+)$/i, "$1 beat $2 $3 for $4"],
  [/^(.+?) defeats (.+?) ([\d-]+).+$/i, "$1 defeated $2 $3"],
  [/^White Sox Roll Into Sunday Eyeing Sweep as Shane Smith Injury Update Surfaces$/i, "A Shane Smith injury update surfaced as the White Sox eyed a Sunday sweep"],
  [/^(.+?) Roll Into (.+?) as (.+?) Injury Update Surfaces$/i, "$3 injury update surfaced as $1 entered $2"],
  [/^(.+?) injury update surfaces$/i, "An injury update surfaced for $1"],
  [/^(.+?) weather: (.+)$/i, "$1 weather update says $2"],
  [/^(.+?) election results: (.+?) defeats (.+?) in (.+)$/i, "$2 defeated $3 in $4"],
  [/^(.+?) joins (.+?) and takes aim at (.+)$/i, "$1 joined $2 and criticized $3"],
  [/^(.+?) takes aim at (.+)$/i, "$1 criticized $2"],
  [/^(.+?) buy homes in (.+?) development$/i, "$1 bought homes in a $2 development"],
  [/^Video shows (.+)$/i, "Video showed $1"],
  [/^The Sad, Wretched Reality of (.+)$/i, "$1 is the focus of a critical local report"],
  [/^BBC .+? Footage of (.+?) at (.+?), Former Reporter Claims$/i, "A former reporter claimed BBC footage showed $1 at $2"],
  [/^ICE at hospital: (.+)$/i, "ICE activity at a hospital involved $1"],
  [/^(.+?) Seen in (.+?) Ahead of (.+)$/i, "$1 were seen in $2 ahead of $3"],
  [/^(.+?) to host (.+)$/i, "$1 will host $2"],
  [/^Viral ['"](.+?)['"] challenge hits (.+)$/i, "A viral $1 challenge hit $2"],
  [/^Could (.+?) find its way upstate.+$/i, "$1 could find its way upstate"],
  [/^(.+?) proposes \$(.+?) (.+)$/i, "$1 proposed a $$$2 $3"],
  [/^Teens storm (.+?) in (.+?) in latest (.+?) incident$/i, "Teens stormed $1 in $2 during a $3 incident"],
  [/^(.+?) Hyper-Focused on (.+?) To Make Team$/i, "$1 is focused on $2 to make the team"],
  [/^(.+?) stars as (.+?) beat (.+)$/i, "$1 starred as $2 beat $3"],
  [/^Warmer temps with (.+?) for (.+?) in (.+)$/i, "Warmer temperatures with $1 are forecast for $2 in $3"],
  [/^(.+?) activate (.+?), DFA (.+)$/i, "$1 activated $2 and designated $3 for assignment"],
  [/^(.+?) reflects on (.+?) after retirement$/i, "$1 reflected on $2 after retirement"],
  [/^(.+?) advocates say (.+)$/i, "$1 advocates said $2"],
  [/^Restrictions on (.+?) advance in (.+)$/i, "Restrictions on $1 advanced in $2"],
  [/^(.+?) booted from (.+)$/i, "$1 was removed from $2"],
  [/^(.+?), IL – Pedestrian Killed After Being Struck by SUV on (.+)$/i, "A pedestrian was killed after being struck by an SUV on $2"],
  [/^(.+?) avoided headlines and stuck to (.+)$/i, "$1 avoided headlines and stuck to $2"],
  [/^(.+?) killed in (.+?) crash$/i, "$1 was killed in a $2 crash"],
  [/^(.+?) Plans (.+?), (.+?), AI Data Center Operations$/i, "$1 plans AI data center operations in $2, $3"],
  [/^Black (.+?) call on reparations from government, (.+)$/i, "Black $1 called for government reparations and $2"],
  [/^The (.+?) First Round Struggles Should Not Come as a Surprise$/i, "$1 first-round struggles were framed as unsurprising"],
  [/^[‘'](.+?)['"] Gang Shooter in (.+?) Attack Taken by ICE After Prison Release$/i, "$1 gang shooter from a $2 attack was taken by ICE after prison release"],
  [/^DEA: (.+?) heading to federal prison$/i, "The DEA said $1 are heading to federal prison"],
  [/^(.+?) Future With (.+?) Gets Major Update After (.+)$/i, "$1 future with $2 received an update after $3"],
  [/^Drivers warned ahead of (.+?) demolition$/i, "Drivers were warned ahead of $1 demolition"],
  [/^(.+?) data center project makes sustainability a priority$/i, "$1 data center project is emphasizing sustainability"],
  [/^(.+?) wins top medals at (.+?) in (.+)$/i, "$1 won top medals at $2 in $3"],
  [/^Come With Us: A Night at (.+)$/i, "A local feature looks at a night at $1"],
  [/^(.+?) late blast powers Houston Dynamo over (.+)$/i, "$1 late blast powered Houston Dynamo over $2"],
  [/^Houston Drops Doubleheader to (.+)$/i, "Houston dropped a doubleheader to $1"],
  [/^Texas Sues (.+?) Over Alleged Birth Tourism in Houston$/i, "Texas sued $1 over alleged birth tourism in Houston"],
  [/^Search for woman swept away by waters in (.+?) suspended after (.+)$/i, "The search for a woman swept away by waters in $1 was suspended after $2"],
  [/^[‘']Living in survival mode[’']: (.+)$/i, "A local report described $1 as living in survival mode"],
  [/^(.+?) weather forecast: (.+)$/i, "$1 weather forecast says $2"],
  [/^(.+?) Owner Doesn't Mince Words on (.+)$/i, "$1 owner criticized $2"],
  [/^Free (.+?) offered across (.+?) area for (.+)$/i, "Free $1 were offered across the $2 area for $3"],
  [/^Free (.+?) offered across (.+?) for (.+)$/i, "Free $1 were offered across $2 for $3"],
  [/^Police investigating after man found with gunshot wound in (.+)$/i, "Police investigated after a man was found with a gunshot wound in $1"],
  [/^Police investigating after man found with (.+?) in (.+)$/i, "Police investigated after a man was found with $1 in $2"],
  [/^Police investigating after (.+?) found with (.+?) in (.+)$/i, "Police investigated after $1 was found with $2 in $3"],
  [/^(\d+) injured near (.+?) after (.+)$/i, "$1 people were injured near $2 after $3"],
  [/^Temps to cool down in (.+?) before warming back up$/i, "Temperatures are expected to cool down in $1 before warming back up"],
  [/^Between (.+?) And (.+?) Is (.+?) For (.+)$/i, "$3 sits between $1 and $2 for $4"],
  [/^(\d+) Must-Visit Historic Towns Near (.+)$/i, "$1 historic towns near $2 were highlighted"],
  [/^Juvenile and (\d+) others hurt in (.+?) shooting near (.+)$/i, "A juvenile and $1 others were hurt in a $2 shooting near $3"],
  [/^(.+?) create emergency water reserve, anticipating (.+)$/i, "$1 created an emergency water reserve while anticipating $2"],
  [/^(.+?) to widen controversial ordinance banning (.+)$/i, "$1 moved to widen an ordinance banning $2"],
  [/^Family Intends To Sue (.+?) After (.+)$/i, "A family intends to sue $1 after $2"],
  [/^Teen sentenced after planning (.+?) attack on (.+)$/i, "A teen was sentenced after planning a $1 attack on $2"],
  [/^(.+?) live updates: (.+)$/i, "$1 update says $2"],
  [/^(.+?) rally in downtown (.+?) draws (.+)$/i, "$1 rally in downtown $2 drew $3"],
  [/^(.+?) opens (.+)$/i, "$1 opened $2"],
  [/^(.+?) closes (.+)$/i, "$1 closed $2"],
  [/^(.+?) expands (.+)$/i, "$1 expanded $2"],
  [/^(.+?) launches (.+)$/i, "$1 started $2"],
  [/^(.+?) reports (.+)$/i, "$1 reported $2"],
  [/^(.+?) remains (.+)$/i, "$1 remained $2"],
  [/^(.+?) continues (.+)$/i, "$1 continued $2"],
  [
    /^(.+?) completes extensive guestroom renovation with (.+?) for (?:it's|it’s|its) (.+?) property in (.+)$/i,
    "$1 completed a guestroom renovation at its $3 property in $4",
  ],
];

function stripHeadlinePrefix(value) {
  return cleanText(value)
    .replace(/^(watch|video|listen|live updates?|update|the latest|latest|breaking|photos?|report|match recap|exclusive|tst images|michael smolens):\s*/i, "")
    .replace(/^[a-z\s]+live updates?:\s*/i, "")
    .replace(/\s*:\s*report$/i, "")
    .replace(/\s*,\s*report$/i, "")
    .replace(/\s+-\s+[^-]{2,60}$/i, "")
    .replace(/[!?]+$/g, "")
    .trim();
...
function pickSourceDetail(item) {
  const title = stripHeadlinePrefix(item?.title || "");
  const summary = cleanText(getSourceEvidenceText(item));
...
function repairGrammarArtifacts(value) {
  return cleanText(value)
    .replace(/(?:^|\s)[A-Z][A-Z]+,\s+[A-Z][A-Za-z .'-]+\s+--\s*/g, " ")
    .replace(/\bhas\s+says\b/gi, "says")
    .replace(/\bhave\s+says\b/gi, "say")
    .replace(/\bhad\s+says\b/gi, "said")
    .replace(/\bhas\s+say\b/gi, "says")
    .replace(/\b(\d+)\s+to(\d+)\b/g, "$1 to $2")
    .replace(/\s+\b(clarification|approval|permission|answers|support|funding)\s+from$/i, " $1")
    .replace(/\s+([,.])/g, "$1")
    .replace(/\s+/g, " ")
    .trim();
}

function getTrimEndWord(value) {
  return cleanText(value)
    .toLowerCase()
    .replace(/[^a-z0-9%$]+$/g, "")
    .split(/\s+/)
...
function hasUnsafeTrimEnding(value) {
  const clean = cleanText(value).replace(/[.!?]+$/g, "").trim();
  if (!clean) return false;
  return UNSAFE_TRIM_END_WORDS.has(getTrimEndWord(clean)) ||
    UNSAFE_TRIM_END_PATTERNS.some((pattern) => pattern.test(clean));
}

function backtrackUnsafeEnding(words, minWords = 12) {
  const safeWords = [...words];
  while (safeWords.length > minWords && hasUnsafeTrimEnding(safeWords.join(" "))) {
    safeWords.pop();
  }
  return safeWords;
}
...
function trimToWords(value, maxWords) {
  const clean = repairGrammarArtifacts(value);
  const words = clean.split(/\s+/).filter(Boolean);
  if (words.length <= maxWords) return clean.endsWith(".") ? clean : `${clean}.`;
  const trimmed = backtrackUnsafeEnding(words.slice(0, maxWords))
    .join(" ")
    .replace(/[,:;]$/, "")
    .trim();
  if (!trimmed) return "";
...
function hasSupervisableSourceMaterial(item) {
  ...
  const summaryWords = wordCount(getSourceEvidenceText(item));
...
function getSourceAngle(item) {
  ...
  const summary = removePublisherBoilerplate(getSourceEvidenceText(item));
...
function buildTeacherSourceCandidates(item) {
  const source = removePublisherBoilerplate(pickSourceDetail(item) || getSourceEvidenceText(item));
...
function buildTeacherRescueCandidates(item) {
  ...
  const localTitleCandidates = buildLocalTitleCandidates(item);
  const candidates = [...sourceCandidates, ...localTitleCandidates];
...
function rewriteKnownFeedDetail(clean, item = {}) {
  ...
  match = normalized.match(/^(.+?) says he will remove all tariffs and restrictions on (.+?) in hono[u]?r of (.+)$/i);
  if (match) {
    return `${match[1]} says tariffs and import restrictions on ${match[2]} will be removed. The move is tied to ${match[3]}`;
  }
...
function rewriteDetail(value, item = {}) {
  let clean = repairGrammarArtifacts(softenPhrase(stripHeadlinePrefix(value))).replace(/\.$/, "");
  if (!clean) return "";
...
function buildLocalContextSentences(item) {
  const title = cleanText(item?.title || "");
  if (/humane society|walk for animals|childhood cancer|celebration of champions|volunteers|clean up|cleanup|plastic pellets|nurdles/i.test(title)) {
    return [
      "The community impact is the reason it belongs in the city feed.",
      "Residents following local service or charity work get the clearest value.",
      "The local audience is families, volunteers, or community groups.",
      "The city connection is community support rather than politics or crime.",
    ];
  }
  if (/medical center|hospital|health|kratom|cancer|clinic|patients/i.test(title)) {
    return [
      "Patients and families are the clearest local audience.",
      "The city-level impact centers on health, care, or access.",
      "Residents get the health-related local effect without added speculation.",
      "The practical concern is local care, safety, or public health.",
    ];
  }
  if (/padres|san diego fc|dodgers|white sox|mlb|mls|soccer|basketball|football|baseball|playoff|matchup|sweep|\bwin\b|\bloss\b/i.test(title)) {
    return [
      "Fans can follow the team impact without leaving the local feed.",
      "The result or roster move matters most to the city sports crowd.",
      "The sports angle is the team change or matchup named here.",
      "Local sports readers get the matchup or team change first.",
    ];
  }
  if (/fire|evacuation|crash|collision|shooting|police|cal fire|injur|safety|disaster|fatalit/i.test(title)) {
    return [
      "Nearby residents can track the public-safety impact first.",
      "The clearest local concern is safety for surrounding communities.",
      "Drivers, families, or nearby neighborhoods may be directly affected.",
      "The update keeps the resident impact at the center.",
    ];
  }
  if (/housing|house prices|homes|real estate|rent|hotel|property|renovation|business|budget|deficit|jobs|workers|lease|market|taxpayer/i.test(title)) {
    return [
      "Residents and workers get the clearest money or business impact.",
      "The city impact centers on costs, property, jobs, or local business.",
      "Homeowners, renters, or consumers are the most relevant local audience.",
      "The update is most useful for people watching local costs.",
    ];
  }
  if (/school|student|teacher|campus|college|university|superintendent|uc\b/i.test(title)) {
    return [
      "Families and students are the clearest local audience.",
      "The education impact is the main reason this belongs in local coverage.",
      "Campus or classroom effects matter most for nearby readers.",
      "The local school community gets the most direct context.",
    ];
  }
  if (/road|highway|airport|transit|traffic|travel|tourism/i.test(title)) {
    return [
      "Travelers and commuters are the main local audience.",
      "The practical impact is movement through the city or nearby routes.",
      "The update is most useful for people watching travel access.",
      "Local transportation and visitor activity are the clearest angle.",
    ];
  }
  if (/festival|concert|show|movie|artist|theater|filmmaker|event|scrippshenge|landmark/i.test(title)) {
    return [
      "The community angle is arts, culture, or a local gathering.",
      "Local audiences get the event note without extra clutter.",
      "The story fits the city feed because it affects culture or attendance.",
      "Residents following local events get the clearest value here.",
    ];
  }
  return [
    "The city link is the clearest confirmed detail.",
    "The local value is the city connection named in the headline.",
    "Readers get the known city detail without added speculation.",
    "The item remains in local coverage because the city link is explicit.",
  ];
}

function buildLocalContextSentence(item) {
  return buildLocalContextSentences(item)[0] || "";
}

function buildLocalTitleCandidates(item) {
  if (String(item?.category || "").toLowerCase() !== "local") return [];
  const eventSentence = rewriteHeadline(item?.title || "");
  if (!eventSentence) return [];
  const contextSentences = buildLocalContextSentences(item);
  const candidates = [];
  if (wordCount(eventSentence) >= 18) candidates.push(eventSentence);
  if (wordCount(eventSentence) >= 5) {
    for (const contextSentence of contextSentences) {
      candidates.push(composeSummary(eventSentence, contextSentence));
      candidates.push(composeSummary(contextSentence, eventSentence));
    }
  }
  return Array.from(new Set(candidates.filter(Boolean)));
}

function isLocalThinHeadlineOnly(item) {
  if (String(item?.category || "").toLowerCase() !== "local") return false;
  const titleVariants = [
    normalizeForCompare(item?.title || ""),
    normalizeForCompare(stripHeadlinePrefix(item?.title || "")),
  ].filter(Boolean);
  const summary = normalizeForCompare(item?.summary || item?.sourceSummary || "");
  if (!titleVariants.length || !summary) return false;
  return titleVariants.some((title) => summary === title || summary.startsWith(title));
}

function getResearchFacts(item) {
  const facts = Array.isArray(item?.summaryResearch?.facts) ? item.summaryResearch.facts : [];
  const title = normalizeForCompare(item?.title || "");
  const rawSummary = normalizeForCompare(item?.summary || item?.sourceSummary || "");
  const seen = new Set();
  return facts
    .map((fact) => removePublisherBoilerplate(fact))
    .filter(Boolean)
    .filter((fact) => {
      const normalized = normalizeForCompare(fact);
      if (!normalized || seen.has(normalized)) return false;
      seen.add(normalized);
      if (normalized === title || normalized.includes(title)) return false;
      if (/you are the public|gift today|direct investment|keeps everyone informed/i.test(fact)) return false;
      if (normalized === rawSummary) return false;
      return wordCount(fact) >= 7;
    });
}

function buildResearchFactCandidates(item) {
  const facts = getResearchFacts(item);
  const candidates = [];
  const rewritten = facts
    .map((fact) => rewriteDetail(fact, item))
    .filter(Boolean)
    .filter((fact) => !repeatsOriginalTitle(item, fact));

  for (const fact of rewritten) {
    if (wordCount(fact) >= 18) candidates.push(fact);
  }

  for (let index = 0; index < rewritten.length; index += 1) {
    const first = rewritten[index];
    const second = rewritten.find((candidate, candidateIndex) => {
      if (candidateIndex === index) return false;
      return normalizeForCompare(candidate) !== normalizeForCompare(first);
    });
    if (!first || !second) continue;
    candidates.push(composeSummary(first, compactDetailSentence(second)));
    candidates.push(composeSummary(compactDetailSentence(first), second));
  }
...
function buildCandidates(item) {
  ...
  const localTitleCandidates = buildLocalTitleCandidates(item);
  const localThinHeadlineOnly = isLocalThinHeadlineOnly(item);
  if (localThinHeadlineOnly) {
    return localTitleCandidates.length ? [...localTitleCandidates, FALLBACK_SUMMARY] : [FALLBACK_SUMMARY];
  }
  if (!detailSentence) return [...localTitleCandidates, FALLBACK_SUMMARY];
  ...
  candidates.push(...buildResearchFactCandidates(item));
...
function evaluateSummaryCandidates(item, candidates, options = {}) {
  ...
  const audienceIntelligence = options.audienceIntelligence || deriveAudienceIntelligence(item, options);
  return candidates.map((text, index) => ({
    text,
    index,
    audienceScore: scoreSummaryForAudience(text, audienceIntelligence),
...
function pickBestPassingCandidate(evaluated, audienceIntelligence) {
  return evaluated
    .filter((candidate) => candidate.text !== FALLBACK_SUMMARY && candidate.evaluation.passed)
    .sort((a, b) => {
      const audienceDelta = scoreSummaryForAudience(b.text, audienceIntelligence) - scoreSummaryForAudience(a.text, audienceIntelligence);
...
function getCandidateDiagnostics(evaluated, limit = 8) {
  return evaluated
    ...
    .map((candidate) => ({
      ...
      audienceScore: candidate.audienceScore,
      metrics: candidate.evaluation.metrics,
    }));
}

function buildSummarySuggestions(item, options = {}) {
  const recentSummaries = options.recentSummaries || [];
  const styleMemory = getStyleMemory(options);
  const audienceIntelligence = deriveAudienceIntelligence(item, options);
  const evaluated = evaluateSummaryCandidates(item, buildCandidates(item), {
    recentSummaries,
    avoidPhrases: styleMemory.avoidPhrases || [],
    audienceIntelligence,
  });
  const hasPassingCandidate = evaluated.some((candidate) => candidate.text !== FALLBACK_SUMMARY && candidate.evaluation.passed);
  const teacherEvaluated = !hasPassingCandidate && hasSupervisableSourceMaterial(item) && !isLocalThinHeadlineOnly(item)
    ? evaluateSummaryCandidates(item, buildTeacherRescueCandidates(item), {
        recentSummaries,
        avoidPhrases: styleMemory.avoidPhrases || [],
        audienceIntelligence,
      })
    : [];
  return [...evaluated, ...teacherEvaluated]
    .filter((candidate) => candidate.text !== FALLBACK_SUMMARY)
    .map((candidate) => ({
      text: candidate.text,
    ...
    .sort((a, b) => {
      ...
      const audienceDelta = b.audienceScore - a.audienceScore;
      if (audienceDelta !== 0) return audienceDelta;
      return (a.failures?.length || 0) - (b.failures?.length || 0);
    })
...
function buildLiveNewsSummary(item, options = {}) {
  ...
  const humanWinner = pickBestPassingCandidate(evaluated, audienceIntelligence);
  ...
  if (!humanWinner && hasSupervisableSourceMaterial(item) && !isLocalThinHeadlineOnly(item)) {
    ...
    const teacherWinner = pickBestPassingCandidate(teacherEvaluated, audienceIntelligence);
...
function applyLiveNewsSummary(item, options = {}) {
  ...
  const summaryAgent = {
    ...
    audience: {
      ...result.audienceIntelligence,
      selectedScore: scoreSummaryForAudience(result.text, result.audienceIntelligence),
    },
...
function applyLiveNewsSummariesToPayload(payload = {}, options = {}) {
  ...
  const spotlightInput = [
    { key: "topStoryOfDay", item: payload.topStoryOfDay },
    { key: "topStoryOfWeek", item: payload.topStoryOfWeek },
  ].filter((entry) => entry.item);
  const spotlightResults = applyLiveNewsSummariesToItems(
    spotlightInput.map((entry) => entry.item),
    {
  ...
  const summarizedSpotlights = spotlightInput.reduce((acc, entry, index) => {
    acc[entry.key] = spotlightResults[index] || entry.item;
    return acc;
  }, {});
  ...
  const summaryItemsByKey = new Map();
  for (const item of [...Object.values(summarizedSpotlights), ...topStories, ...feed]) {
    const key = getSummaryCacheKey(item) || item.id || item.link || item.title;
    if (key && !summaryItemsByKey.has(key)) {
      summaryItemsByKey.set(key, item);
    }
  }
  return {
    ...payload,
    topStoryOfDay: summarizedSpotlights.topStoryOfDay || payload.topStoryOfDay || null,
    topStoryOfWeek: summarizedSpotlights.topStoryOfWeek || payload.topStoryOfWeek || null,
    topStories,
    feed,
    summaryHealth: getSummaryHealth(Array.from(summaryItemsByKey.values())),
  };
}
...
function getSummaryHealth(items = []) {
  ...
  return {
    ...
    needsReviewCount,
    audienceIntelligence: getAudienceHealth(list),
    styleCounts,
  };
}
