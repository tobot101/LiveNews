...
function buildNewsArticleSchema(story, origin) {
  ...
  return {
    ...
    description: story.metaDescription || story.description || story.summaryShort || story.dek,
...
function renderPublicStoryPage(story, options = {}) {
  ...
  const title = story.metaTitle || `${storyTitle} | Live News`;
  const description = story.metaDescription || story.description || story.summaryShort || story.dek || "";
  const imageUrl = absoluteUrl(origin, story.thumbnailUrl || "/android-chrome-512x512.png");
