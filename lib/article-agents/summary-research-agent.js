...
function extractParagraphEvidence(html) {
  ...
  for (const match of cleanedHtml.matchAll(/<(?:p|h1|h2)\b[^>]*>([\s\S]*?)<\/(?:p|h1|h2)>/gi)) {
...
function getStoryTokenSet(item, sourceResearch = null) {
...
function getSharedStoryTokenCount(value, storyTokens) {
  const seen = new Set();
  let shared = 0;
  for (const token of tokenize(value)) {
    if (seen.has(token)) continue;
    seen.add(token);
    if (storyTokens.has(token)) shared += 1;
  }
  return shared;
}

function filterStoryRelevantFacts(facts, item, sourceResearch = null, { strict = false } = {}) {
  const storyTokens = getStoryTokenSet(item, sourceResearch);
  if (!storyTokens.size) return uniqueEvidence(facts, DEFAULT_MAX_FACTS);
  const minimumShared = strict ? 2 : 1;
  return uniqueEvidence(facts, DEFAULT_MAX_FACTS).filter((fact) => {
    const shared = getSharedStoryTokenCount(fact, storyTokens);
    if (shared >= minimumShared) return true;
    const factTokens = tokenize(fact);
    return !strict && factTokens.length >= 10 && shared >= 1;
  });
}
...
function buildRelatedFacts(item) {
  ...
  return uniqueEvidence(
    ...
    const shared = factTokens.filter((token) => currentTitleTokens.has(token)).length;
    return shared >= Math.min(2, currentTitleTokens.size || 2);
  });
}

function mergeResearchEvidence(item, sourceResearch, topicResearch = null) {
  const sourceFacts = filterStoryRelevantFacts(sourceResearch?.facts || [], item, sourceResearch);
  const topicFacts = filterStoryRelevantFacts(topicResearch?.facts || [], item, sourceResearch, { strict: true });
  const relatedFacts = filterStoryRelevantFacts(buildRelatedFacts(item), item, sourceResearch, { strict: true });
  const facts = sourceFacts.length
    ? uniqueEvidence(sourceFacts, DEFAULT_MAX_FACTS)
    : uniqueEvidence([...topicFacts, ...relatedFacts], DEFAULT_MAX_FACTS);
  const stages = [
  ...
  const readyFacts = facts.length || topicFacts.length || relatedFacts.length;
  return {
    ...
    stages,
    status: readyFacts ? "ready" : "needs_review",
    reason: readyFacts ? "" : sourceResearch?.reason || "no_research_evidence",
  };
}
