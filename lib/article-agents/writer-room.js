...
function rewriteMemoryOptions(options = {}) {
  return {
    storePath: options.rewriteMemoryStorePath,
    limit: options.rewriteMemoryLimit || 8,
  };
}

function safeRewriteLessonView(record = {}) {
  return {
    id: cleanText(record.id),
    category: cleanText(record.category),
    fieldName: cleanText(record.fieldName),
    failedTeacherNames: uniqueClean(record.failedTeacherNames || []),
    lesson: cleanText(record.lesson),
    rewriteStrategyUsed: cleanText(record.rewriteStrategyUsed),
    weakOutputShape: cleanText(record.weakOutputShape),
    approvedOutputShape: cleanText(record.approvedOutputShape),
    beforeScore: Number(record.beforeScore || 0),
    afterScore: Number(record.afterScore || 0),
    copyRiskBefore: Number(record.copyRiskBefore || 0),
    copyRiskAfter: Number(record.copyRiskAfter || 0),
  };
}
...
function failedTeacherNamesFromCandidate(candidate = {}) {
  return uniqueClean(
    asArray(candidate.teacherChecks || [])
      .filter((teacher) => teacher?.blocking || teacher?.passed === false || Number(teacher?.score || 0) < 70)
      .map((teacher) => teacher.name || teacher)
  );
}

function collectRewriteMemoryGuidance(factMap = {}, fieldName = "description", options = {}, failure = {}) {
  return uniqueLessons(collectRewriteLessons({
    category: factMap.sourceCategory || "Top",
    fieldName,
    failedTeacherNames: failure.failedTeacherNames || [],
    copyRiskType: failure.copyRiskType,
  }, rewriteMemoryOptions(options)));
}
...
function runWriterRoom(story = {}, fieldName = "description", options = {}) {
  ...
  let rewriteLessonsUsed = collectRewriteMemoryGuidance(factMap, fieldName, options);
  const agentNotes = [
    ...
    {
      agent: "RewriteMemoryAgent",
      status: rewriteLessonsUsed.length ? "ready" : "empty",
      notes: rewriteLessonsUsed.length
        ? rewriteLessonsUsed.slice(0, 3).map((record) => `Safe lesson: ${record.lesson}`)
        : ["No approved rewrite lessons matched this category or field yet."],
    },
  ];
  if (!validation.ready) {
    return {
  ...
  if (managingEditor.status !== "passed" && managingEditor.selectedCandidate) {
    const failureLessons = collectRewriteMemoryGuidance(factMap, fieldName, options, {
      failedTeacherNames: failedTeacherNamesFromCandidate(managingEditor.selectedCandidate),
      copyRiskType: managingEditor.selectedCandidate.copyRisk?.risk || "copy_risk",
    });
    rewriteLessonsUsed = uniqueLessons([...rewriteLessonsUsed, ...failureLessons]);
    if (failureLessons.length) {
      agentNotes.push({
        agent: "RewriteMemoryAgent",
        status: "used_for_rewrite",
        notes: failureLessons.slice(0, 3).map((record) => `Rewrite guidance: ${record.lesson}`),
      });
    }
